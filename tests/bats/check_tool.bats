...
setup() {
  setup_repo_env
  CHECK_TOOL="${REPO_ROOT}/scripts/lib/check_tool.sh"
}
...
@test "bazel_checkout_root points at MODULE.bazel" {
  run bash -c "source '${CHECK_TOOL}'; bazel_checkout_root"
  [ "$status" -eq 0 ]
  [[ -f ${output}/MODULE.bazel ]]
}
