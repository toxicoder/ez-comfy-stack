...
4. Chatterbox Multilingual V3 (MIT, PerTh on) — ISO `language_id` (`es`, not `Spanish`). **Clone CFG** auto is 0 on EN→ES (less English accent); 0.5 same-language. Duration lock pitch-preserves up to 1.25× (ffmpeg `atempo`), then spills, then fade-trims — it does not crush a long clone into the original window. Raw clones: `dubs/<slug>/render/turn_NNNN.raw.wav`. Needs the complete snapshot (`ve.pt`, `s3gen.pt`, T3 V3, tokenizer JSON, `conds.pt`), not t3-only `comfy/tts`, and a wheel whose `from_local` accepts `t3_model=v3` (GitHub pin, not PyPI 0.1.7). Qwen3-TTS Base clones from the same refs (`download-podcast --tier qwen3tts`). Missing ASR/clone/llama.cpp: empty mix + **Dub status** (never the original recording). Clone lines longer than 300 characters are split.
