...
def test_dub_localize_graph() -> None:
    ...
    assert script["widgets_values"][5] == "analyze"
    seed = json.loads(script["widgets_values"][0])
    assert seed["turns"] == []
    assert seed["stage"] == "analyze"
    ...
    assert render["widgets_values"][2] is False
    ...
    assert "Stage analyze" in note or "Stage **analyze**" in note
