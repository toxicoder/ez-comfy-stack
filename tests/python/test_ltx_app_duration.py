"""Standalone LTX Apps default to 10.00 s / 241 frames; films stay 5.00 s."""
...
def test_standalone_ltx_apps_are_ten_seconds() -> None:
    apps: list[str] = []
    for path in lab_graph_paths():
        ...
        assert "10 seconds, 24 fps" in blob or "Ten seconds." in blob or rel.endswith(
            "-10s"
        )
    assert "stills/talking-head" in apps
    assert "motion/av/still-to-video-10s" in apps
...
def test_ltx_av_subgraph_is_ten_seconds() -> None:
    path = SUBS / "ltx-av-10s.json"
    assert path.is_file()
