...
def test_required_loadimage_is_first_app_widget() -> None:
    ...
    flf = _widget_names(_load("motion/av/first-last-10s.json"))
    assert flf[0] == "image"
    assert flf[1] == "image"
...
def test_i2v_hides_style_t2v_keeps_it() -> None:
    ...
    ltx_i2v = _widget_names(_load("motion/av/still-to-video-10s.json"))
    assert "style" not in ltx_i2v
    t2v = _widget_names(_load("motion/silent/text-to-video-5s.json"))
    assert "style" in t2v
    ltx_t2v = _widget_names(_load("motion/av/text-to-video-10s.json"))
