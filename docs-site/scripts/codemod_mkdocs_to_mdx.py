...
_COMPONENT_TAG_RE = re.compile(
    r"</?(?:Callout|Tabs|Tab|Cards|Card|EzCommand|EzCmd|GlossaryBody|PageBrief)\b(?P<attrs>[^>]*?)/?>|"
    r"<div\b[^>]*>|</div>",
    re.IGNORECASE,
)
_EZCMD_OPEN_RE = re.compile(r"^(?P<indent>[ \t]*)```ezcmd[ \t]*$")
_EZCMD_ID_RE = re.compile(r"^id:\s*([a-z0-9-]+)\s*$")
_GLOSSARY_PLACEHOLDER_RE = re.compile(r"<!--\s*ez-glossary:render\s*-->")
#: Attributes whose values carry body text and must stay in the prose projection.
_PROSE_ATTR_RE = re.compile(r'(?P<name>title|value|id)=(?P<quote>["\'])(?P<value>[^"\']*)\2')
_EZCMD_BLOCK_RE = re.compile(r"(?ms)^[ \t]*```ezcmd[^\n]*\n(.*?)```[ \t]*$")
_PROSE_LINE_RULES = (
...
def _transform_ezcmd(lines: list[str]) -> list[str]:
    ...
    Returns:
        Lines with ezcmd fences rewritten.
    """
    fenced = _fenced_ranges(lines)
    out: list[str] = []
    index = 0
    while index < len(lines):
        match = _EZCMD_OPEN_RE.match(lines[index])
        ...
        while end < len(lines) and not lines[end].startswith("```"):
            id_match = _EZCMD_ID_RE.match(lines[end].strip())
            if id_match:
                cmd_id = id_match.group(1)
            end += 1
        if not cmd_id or end >= len(lines):
            out.append(lines[index])
            index += 1
            continue
        out.append(f'{indent}<EzCommand id="{cmd_id}" />')
        index = end + 1
    return out


def _transform_glossary_placeholder(lines: list[str]) -> list[str]:
    """Replace the glossary render comment with ``<GlossaryBody />``.
    ...
    """
    return [
        "<GlossaryBody />" if _GLOSSARY_PLACEHOLDER_RE.search(line) else line for line in lines
    ]
...
def transform_text(text: str) -> str:
    ...
    """
    lines = text.split("\n")
    lines = _transform_glossary_placeholder(lines)
    lines = _transform_ezcmd(lines)
...
_EMITTED_TAG_RE = re.compile(
    r"^[ \t]*</?(?:Callout|Tabs|Tab|Cards|Card|EzCommand|EzCmd|GlossaryBody|PageBrief|Mermaid|abbr|include)\b",
    re.IGNORECASE,
)
...
def prose_projection(text: str) -> str:
    ...
    """
    def _ezcmd_prose(match: re.Match[str]) -> str:
        """Keep recipe ids from ezcmd fences so they match ``<EzCommand id>``."""
        ids = re.findall(r"(?m)^id:\s*([a-z0-9-]+)\s*$", match.group(1))
        return " " + " ".join(ids) + " "

    body = _EZCMD_BLOCK_RE.sub(_ezcmd_prose, text)
