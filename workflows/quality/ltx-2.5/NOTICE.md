...
Lab **standalone LTX Apps default to 12.00 s / 289 frames / 1280×704** (`motion/av/still-to-video-10s.json` and showcase Apps). **90s one-click films stay 5.00 s / 121 frames** (`motion/av/still-to-shot.json`). Showcase Apps (`motion/av/dialogue-10s`, `motion/av/multishot-10s`, `motion/av/product-hero`, `motion/av/first-last-10s`, `motion/av/audio-to-video-10s`) use the 12 s envelope with core nodes only (`LTXVModalityGuidance`, `LTXVAddGuide` / `LTXVCropGuides`, `LTXVAudioVAEEncode`). Duration-head 5 / 8 / 10 / 12 s is allowed behind `ez_film.ltx_timing.preflight_duration_s` (`1+8n` frames; even latents refuse). Do not queue a 30/60/90 s latent on GB10.
...
| IC-LoRA Union Control | Depth / canny / pose v2v. Lab envelope: `workflows/dcc/dcc/depth-control-10s.json`. Official graph stays Templates (`LTX-2.5_ICLoRA_Union_Control_Distilled.json`). LoRA: `download-ltx --tier iclora` → `ltx-2.3-22b-ic-lora-union-control-ref0.5.safetensors`. Distilled-only. Refuse 19B. |
