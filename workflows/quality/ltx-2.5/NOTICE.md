...
Lab **default printers stay 5.00 s / 121 frames / 1280×704** (`ltx/still-to-video-5s.json` and the 90s one-click films). Showcase Apps (`ltx/dialogue-5s`, `ltx/multishot-5s`, `ltx/product-hero`, `ltx/first-last-5s`, `ltx/audio-to-video-5s`) use the same envelope with core nodes only (`LTXVModalityGuidance`, `LTXVAddGuide` / `LTXVCropGuides`, `LTXVAudioVAEEncode`). Duration-head 8 / 10 / 12 s is opt-in behind `ez_film.ltx_timing.preflight_duration_s` (`1+8n` frames; even latents refuse). Do not queue a 30/60/90 s latent on GB10.
...
| IC-LoRA Union Control | Depth / canny / pose v2v. Lab envelope: `workflows/dcc/dcc/ltx/depth-control-5s.json`. Official graph stays Templates (`LTX-2.5_ICLoRA_Union_Control_Distilled.json`). LoRA: `download-ltx --tier iclora` → `ltx-2.3-22b-ic-lora-union-control-ref0.5.safetensors`. Distilled-only. Refuse 19B. |
