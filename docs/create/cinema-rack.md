...
- Copying a family CLIP string into **klein/still-draft**, **wan/t2v-5s**, or **ltx/t2v-5s**
...
Style dropdown on the Enhance nodes stays **none** so `apply_style_to_prompt` does not strip cinema grade language. Use the 50 style presets when you want a medium (anime, watercolor); use Cinema Rack when you want camera/light/time.
...
3. Paste into **klein/still-draft**, **wan/t2v-5s**, or **ltx/t2v-5s**. For I2V, set Family to `wan_i2v` / `ltx_i2v` first so look axes are already omitted.
