...
- **Lyrics** are the bars (rap) or the arrangement **structure** (Drive-through EDM). Section labels `[intro]`, `[verse]`, `[pre-chorus]`, `[chorus]`, `[bridge]`, `[breakdown]`, `[inst]`, `[outro]`, and `[spoken word]` are vocal **hints** operators may add — they are not a rights grant. EDM graphs use empty-body `[drop]` / `[build-up]` / `[breakdown]` / `[inst]` / `[outro]` with production cues **inside the brackets**. ACE-Step sings any free-text line under a marker. Ninety-nine takes stay instrumental (encoder language `unknown`). Two bass-set treats add one 1–2 word `[chorus]` DJ chop (never `[verse]`). Do not put the act name in the score — ACE will sing it. Length, meter, and key are encoder fields (one each per take), not a second timeline.
