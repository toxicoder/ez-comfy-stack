...
description: Seeded Wan 2.2 silent ~5 s graphs, LTX-2.5 AV ~8 s heroes, GIF/bumper loops, and creator motion plates.
...
- **AV hero (LTX-2.5)** I2V / T2V and the div32 size warning
...
- **Queuing a silent ~5 s clip** or an AV ~8 s hero from a seeded filename
...
| --- | --- |
| **[motion/silent/still-to-video-5s](../generated/workflows/motion/silent/still-to-video-5s.md)** | Silent I2V smoke. Default 832x480; **Format / platform** (YouTube 16:9 / Shorts 9:16 / Custom). **121** frames @ 24 fps. MagCache **draft-only** (`extra.lab_magcache`) |
...
| Workflow | What it does |
| --- | --- |
| **[motion/av/still-to-video-8s](../generated/workflows/motion/av/still-to-video-8s.md)** | ~8 s I2V. Default **1280x704**; **Format / platform** (LTX 16:9 / 9:16 / Custom, never 720). Native audio (Community License, $10M cap) |
| **[motion/av/clip-chain](../generated/workflows/motion/av/clip-chain.md)** | Four 8.00 s LTX AV beats, last-frame continuity, `EZClipConcat` master. Occupancy **ltx**. Prefix `ez_clip_b01`...`b04`. Match-input Format. Enhance on. No Klein |
| **[motion/av/text-to-video-8s](../generated/workflows/motion/av/text-to-video-8s.md)** | ~8 s T2V AV. Default **1280x704**; same Format / platform combo |

---

## Showcase (LTX-2.5)

Authored recipes. Prompt Enhance **off**. Occupancy **ltx**. Same 8.00 s / 193 / **1280x704** printer as the heroes. Two-stage DFR stays in Comfy **Templates -> LTX-2.5**.

| Workflow | What it does |
| --- | --- |
| **[motion/av/dialogue-8s](../generated/workflows/motion/av/dialogue-8s.md)** | T2V with a quoted spoken line + interleaved foley. `LTXVModalityGuidance` (A/V coupling). Prefix `ez_ltx_dialogue`. Mouths will not match |
| **[motion/av/multishot-8s](../generated/workflows/motion/av/multishot-8s.md)** | Native multishot T2V: named hard cut + match cut in one 8 s clip. Prefix `ez_ltx_multishot` |
...
| **[motion/av/first-last-8s](../generated/workflows/motion/av/first-last-8s.md)** | First-last-frame AV. Two Klein stills pin start/end via `LTXVAddGuide` on the video latent (before audio concat). Prefix `ez_ltx_flf` |
| **[motion/av/audio-to-video-8s](../generated/workflows/motion/av/audio-to-video-8s.md)** | Audio freeze: `LoadAudio` -> encode into the joint latent; MP4 muxes the **original** wav (no audio VAE decode). Prefix `ez_ltx_a2v`. Drop `ez_a2v_bed.wav` in `${COMFY_OUTPUT_DIR}/input` |

<Callout type="warning" title="LTX width/height must be divisible by 32">
  Broadcast 720p (**1280x720**) and 1080p (**1920x1080**) are **not** native LTX VAE sizes (720/16=45, then the next `/2` patch fails). Lab landscape graphs use **1280x704**. Klein **I2V feeders** (`klein-still-hero`, 90s film identity) are **1280x704**. Thumbnails/end-cards may stay 1280x720. On LTX Apps pick **Format / platform -> LTX - 16:9 YouTube (1280x704)** instead of typing 720. Typing 720 or 1080 on LTX widgets is **auto-snapped** (704 / 1056) by `ez_ltx_spatial` - prefer 704 so you skip the extra crop. Portrait shorts I2V is **768x1280**. See [Troubleshooting](../troubleshooting.md).
</Callout>
---
...
| **[motion/av/shorts-still-8s](../generated/workflows/motion/av/shorts-still-8s.md)** | Vertical AV I2V (~8 s) with world audio |
| **[motion/loops/bumper-loop](../generated/workflows/motion/loops/bumper-loop.md)** | Loopable MP4 bumper (ping-pong) |
| **[motion/av/broll-ambient](../generated/workflows/motion/av/broll-ambient.md)** | Ambient B-roll AV plate (~8 s) |
...
| **[motion/av/hook-av](../generated/workflows/motion/av/hook-av.md)** | ~8 s AV cold open |
...
## Duplicate clip-chain beats (5-24)

Shipped **[motion/av/clip-chain](../generated/workflows/motion/av/clip-chain.md)** is four beats. Duplicate **Beat** groups on the canvas for longer chains. App Mode will not pick up Beat 5+ prompts. 4 x 8 s sequential prints are tens of minutes on GB10. Occupancy **ltx** XOR.
...
Past 24 stems: host `./scripts/utilities/concat-shots.sh --files a.mp4,b.mp4 --cap-seconds <sum> --yes` (the default `--files` cap is 90 s).
