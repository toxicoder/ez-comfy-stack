...
| `setup-client` | Laptop bootstrap: SSH Host ez-spark + LocalForward. Does not start Comfy. [This computer](start/this-computer.md) | Start Compose from the laptop; skip WSL on Windows |
| `onboard` | Spark: `setup --install-docker` then doctor. Laptop: `setup-client` then remote clone/setup/doctor. Never auto-start. [Onboard](start/onboard.md) | Treat this as `start` |
| `spark-ui` | Foreground SSH LocalForward to Host ez-spark (stack must already be started) | Bind `0.0.0.0`; start Compose from the laptop |
...
| `download-llm [--tier enhance\|qwen36-35b-a3b\|describe\|all] [--limit auto\|N\|off]` | Default `enhance` is the 4B GGUF (already in `download-models`). `qwen36-35b-a3b` is opt-in ~23 GB for occupancy `llm-desk`. `describe` is the opt-in caption VLM. [Tiers](download-tiers.md) | Treat 35B or describe as part of `download-models`; GPU-offload the in-canvas 4B |
...
| `export-guides` | Dump a 1280x704 (or 768x1280) / 120f clay+depth+canny pack; dies if Comfy is heavy. `--print ltx-iclora-depth\|ltx-iclora-canny\|wan-flf` | 1280x720; dump while Comfy is unparked |
...
| `film-accept` | Fail-closed 90s gate (duration / 1280x704 / LTX audio; world-only speech-band; 90s master when present; stems LUFS when `audio_policy: stems`) | `--skip-accept` as a habit |
...
| Occupancy mode x jobs | [Occupancy matrix](operate/occupancy-matrix.md) |
