...
<Cards>
  <Card title="First install">
    ...
    [Default pack](#default-pack-download-models)
  </Card>
  <Card title="A different still">
    ---

    Klein NVFP4 / base, or Z-Image Turbo. Not “better video”.

    [Image](#image-still-unet)
  </Card>
  <Card title="A different Wan graph">
    ---

    A14B, Fun InP, VACE, or S2V. `--tier all` skips VACE and S2V.

    [Wan](#wan-different-graphs)
  </Card>
  <Card title="LTX or IC-LoRA">
    ---

    Lab default is 2.5. `balanced` / `quality` are 2.3 aliases.

    [LTX](#ltx-generation-not-better)
  </Card>
  <Card title="Podcast, dub, music">
    ---

    Opt-in audio. ACE-Step turbo is shared. Occupancy audio.

    [Audio](#audio-podcast-dub-music)
  </Card>
  <Card title="3D or extras">
    ---
...
| LLM | `enhance` + `qwen36-35b-a3b` + `describe` | putting 35B or the describe VLM into `download-models` |
...
<EzCommand id="download-restore" />

### Prompt-enhance GGUF (already in `download-models`)

Default `--tier enhance` is the 4B Q4_K_M already pulled by `download-models`. `--tier qwen36-35b-a3b` is opt-in (~23 GB) for `occupancy enter llm-desk`. `--tier describe` is the opt-in Qwen2.5-VL-3B caption GGUF + mmproj for **Describe image**. `--tier all` is enhance + 35B + describe. Do not add the 35B or describe packs to `download-models`.

<EzCommand id="download-llm" />
