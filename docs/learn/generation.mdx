...
description: Latents, CLIP, VAE, steps, CFG, and why this studio iterates 5 s Wan / 12 s LTX clips instead of a 90 s denoise.
...
| ~5 s Wan / ~12 s LTX App | LTX Apps **289** frames (`1+8n`) @ 24 fps ≈ **12.04 s**; film printers **121** ≈ **5.04 s**; Wan shot **120** = **5.00 s** | Iterate in minutes |
...
| **A2V** | Soundtrack → picture | Often a freeze still | Input audio | **motion/av/audio-to-video-12s** (talking-head is I2V smoke) |
