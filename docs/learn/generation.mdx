...
description: Latents, CLIP, VAE, steps, CFG, and why this studio iterates 5 s Wan / 8 s LTX clips instead of a 90 s denoise.
...
- Latent size, frame counts, and the LTX div32 rule
...
Typing 720 or 1080 on an LTX widget is auto-snapped (704 / 1056) by `ez_ltx_spatial`. Prefer 704 so you skip the extra crop. The same pack snaps illegal **120** length to **121** (`1+8n`). Portrait shorts I2V is **768x1280**.
...
Next: [Klein, Wan, and LTX](pipeline.md). Prompt craft after the first still: [Prompting](../prompting.md).
