...
`doctor` must **not** print `attention: pytorch-fallback` on a running Spark (10-20x slow). spark-timing is measured on a real Spark after Kitchen is live - CI does not invent seconds.
...
Next: [Architecture](architecture.md). Operator first run: [Getting Started](../getting-started.md). Command catalog: [manage.sh reference](../manage-cli.md).
