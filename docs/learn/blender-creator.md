...
Stay on `:8188` after the dump (no `--install-inputs`): [Stay in Comfy after a Blender dump](comfy-first-blender.md) — **dcc/klein/guide-still**.
...
| Talking-head / VO-locked picture | Union Control. Use A2V freeze (`ltx/audio-to-video-5s`) |
