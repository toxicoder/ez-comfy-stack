...
description: Why the US-safe studio is still → silent 5 s → AV 12 s, and how each model reads a prompt.
...
| **LTX-2.5** distilled INT8-convrot | Joint AV ~12 s Apps (5 s film printers) | Gemma4-with-proj · `ltxv` | Community License, **gated**, $10M company cap |
...
Next: [Why one heavy job](occupancy.md). Playbook: [Still to motion to AV](../visual-generative-ai.md). Filenames: [Workflow catalog](../studio-workflows.md).
