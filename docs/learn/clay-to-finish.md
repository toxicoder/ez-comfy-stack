...
7. Stop Klein. `download-ltx --tier iclora` if needed. Queue **dcc/depth-control-5s** or Templates → LTX-2.5 Union Control (depth from `depth.mp4`). MagCache off. Distilled-only. Refuse 19B Union.
...
| Talking-head / VO-locked picture | Union Control. Use A2V freeze (`motion/av/audio-to-video-5s`). Mouths will not match. |
