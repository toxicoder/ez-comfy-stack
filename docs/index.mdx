...
- Documentation map (Learn / Get started / Studio / Operate / Reference / Contribute)
...
<Cards>
  ...
  <Card title="Make something">
    ...
    Still → silent 5 s → AV 12 s, or pick a thumbnail / GIF / 90s film from the catalog.
...
flowchart TB
  ...
  Learn --> L1["syllabus · ComfyUI · pipeline · occupancy idea · glossary"]
  Start --> S1["Getting Started · licenses · FAQ"]
  Studio --> C1["still → motion → AV · catalogs · playbooks"]
  Operate --> O1["daily loop · occupancy desk · troubleshooting"]
  Ref --> R1["manage.sh · occupancy matrix · node encyclopedia"]
...
| **Still → silent 5 s → AV 12 s** | [Still to motion to AV](visual-generative-ai.md) |
