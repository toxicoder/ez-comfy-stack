...
=== "Audio (podcast / dub / rap / EDM)"
    ...
    | Workflow | What it does |
    | --- | --- |
    | **dub-localize-lab-example** | Multi-speaker clone-and-translate. Pick or upload source media. Rights gate. **Clone CFG** auto 0 on EN→ES. Duration-locked `ez_dub_yt` for YouTube Languages. Prefix `ez_dub_mix` |
    ...
    Drive-through EDM pack (warped hybrid-trap bass set, not rap over a club bed). Queue on its own; table order is the set list (`phase0/` hour 1, `phase1/` hour 2, `phase2/` hour 3 headliner, `phase3/` hour 4 afterparty, `phase4/` Secret Homage). Drop first, hard warpy drops, trap drums, chest-sub bass, no quiet dips. Phase2, phase3, and phase4 vary Comfy node placement across five layouts. Full table: [Local music](music.md).

    | Workflow | What it does |
    | --- | --- |
    | **music-edm-drive-through-night-window-lab-example** | **180 s** hybrid trap 145 (`Drive-through - Night Window - v0`). |
    | **music-edm-drive-through-open-lane-lab-example** | **180 s** riddim 152 (`Drive-through - Open Lane - v0`). |
    | **music-edm-drive-through-exit-seven-lab-example** | **180 s** tearout 142 (`Drive-through - Exit Seven - v0`). |
    | **music-edm-drive-through-skyline-pass-lab-example** | **180 s** brostep 145 (`Drive-through - Skyline Pass - v0`). |
    | **music-edm-drive-through-on-ramp-lab-example** | **180 s** wave bass 155 (`Drive-through - On-Ramp - v0`). |
    | **music-edm-drive-through-tunnel-bass-lab-example** | **180 s** dirty bass 150 (`Drive-through - Tunnel Bass - v0`). |
    | **music-edm-drive-through-wide-open-lab-example** | **180 s** color bass 150 (`Drive-through - Wide Open - v0`). DJ shout treat |
    | **music-edm-drive-through-overpass-lab-example** | **180 s** dirty dubstep 150 (`Drive-through - Overpass - v0`). |
    | **music-edm-drive-through-second-wave-lab-example** | **180 s** hybrid trap 150 (`Drive-through - Second Wave - v0`). DJ shout treat |
    | **music-edm-drive-through-freight-pulse-lab-example** | **180 s** drumstep 176 (`Drive-through - Freight Pulse - v0`). |
    | **music-edm-drive-through-keep-going-lab-example** | **180 s** neuro bass 170 (`Drive-through - Keep Going - v0`). |
    | **music-edm-drive-through-horizon-kick-lab-example** | **180 s** tearout 165 (`Drive-through - Horizon Kick - v0`). |
    | **music-edm-drive-through-clean-wreckage-lab-example** | **180 s** brostep 150 (`Drive-through - Clean Wreckage - v0`). |
    | **music-edm-drive-through-heart-lane-lab-example** | **180 s** wave bass 145 (`Drive-through - Heart Lane - v0`). |
    | **music-edm-drive-through-dawn-receipt-lab-example** | **180 s** chest bass 140 (`Drive-through - Dawn Receipt - v0`). |
    | **music-edm-drive-through-rumble-strip-lab-example** | **180 s** hybrid trap 140 (`Drive-through - Rumble Strip - v1`). |
    | **music-edm-drive-through-low-lane-lab-example** | **180 s** riddim 144 (`Drive-through - Low Lane - v1`). |
    | **music-edm-drive-through-warm-merge-lab-example** | **180 s** wave bass 148 (`Drive-through - Warm Merge - v1`). |
    | **music-edm-drive-through-colour-span-lab-example** | **180 s** color bass 150 (`Drive-through - Colour Span - v1`). |
    ...
    | **music-edm-drive-through-jump-bay-lab-example** | **180 s** brostep 150 (`Drive-through - Jump Bay - v1`). |
    | **music-edm-drive-through-psy-median-lab-example** | **180 s** neuro bass 145 (`Drive-through - Psy Median - v1`). |
    | **music-edm-drive-through-groove-mile-lab-example** | **180 s** hybrid trap 144 (`Drive-through - Groove Mile - v1`). |
    | **music-edm-drive-through-donk-ramp-lab-example** | **180 s** tearout 150 (`Drive-through - Donk Ramp - v1`). |
    | **music-edm-drive-through-bounce-booth-lab-example** | **180 s** chest bass 140 (`Drive-through - Bounce Booth - v1`). |
    | **music-edm-drive-through-toll-growl-lab-example** | **180 s** riddim 150 (`Drive-through - Toll Growl - v1`). |
    | **music-edm-drive-through-night-oil-lab-example** | **180 s** hybrid trap 142 (`Drive-through - Night Oil - v1`). |
    | **music-edm-drive-through-chest-pass-lab-example** | **180 s** dirty bass 150 (`Drive-through - Chest Pass - v1`). |
    | **music-edm-drive-through-sunrise-sub-lab-example** | **180 s** wave bass 140 (`Drive-through - Sunrise Sub - v1`). |
    | **music-edm-drive-through-lantern-merge-lab-example** | **180 s** hybrid trap 150 (`Drive-through - Lantern Merge - v2`). |
    | **music-edm-drive-through-firefly-lane-lab-example** | **180 s** color bass 152 (`Drive-through - Firefly Lane - v2`). |
    | **music-edm-drive-through-canopy-bounce-lab-example** | **180 s** chest bass 148 (`Drive-through - Canopy Bounce - v2`). |
    | **music-edm-drive-through-grove-wreck-lab-example** | **180 s** riddim 150 (`Drive-through - Grove Wreck - v2`). |
    | **music-edm-drive-through-moss-sub-lab-example** | **180 s** dirty bass 150 (`Drive-through - Moss Sub - v2`). |
    | **music-edm-drive-through-fern-stack-lab-example** | **180 s** hybrid trap 155 (`Drive-through - Fern Stack - v2`). |
    | **music-edm-drive-through-pollen-kick-lab-example** | **180 s** drumstep 174 (`Drive-through - Pollen Kick - v2`). |
    | **music-edm-drive-through-cedar-growl-lab-example** | **180 s** tearout 150 (`Drive-through - Cedar Growl - v2`). |
    | **music-edm-drive-through-moon-ramp-lab-example** | **180 s** wave bass 148 (`Drive-through - Moon Ramp - v2`). |
    | **music-edm-drive-through-trail-bounce-lab-example** | **180 s** color bass 150 (`Drive-through - Trail Bounce - v2`). |
    | **music-edm-drive-through-dew-wreck-lab-example** | **180 s** neuro bass 172 (`Drive-through - Dew Wreck - v2`). |
    | **music-edm-drive-through-sap-stack-lab-example** | **180 s** brostep 165 (`Drive-through - Sap Stack - v2`). |
    | **music-edm-drive-through-glade-split-lab-example** | **180 s** dirty dubstep 150 (`Drive-through - Glade Split - v2`). |
    | **music-edm-drive-through-root-chest-lab-example** | **180 s** chest bass 148 (`Drive-through - Root Chest - v2`). |
    | **music-edm-drive-through-ember-crest-lab-example** | **180 s** hybrid trap 165 (`Drive-through - Ember Crest - v2`). |
    | **music-edm-drive-through-brake-fade-lab-example** | **180 s** dirty bass 150 (`Drive-through - Brake Fade - v3`). |
    | **music-edm-drive-through-diesel-hum-lab-example** | **180 s** hybrid trap 152 (`Drive-through - Diesel Hum - v3`). |
    | **music-edm-drive-through-axle-grind-lab-example** | **180 s** tearout 155 (`Drive-through - Axle Grind - v3`). |
    | **music-edm-drive-through-weigh-station-lab-example** | **180 s** brostep 158 (`Drive-through - Weigh Station - v3`). |
    | **music-edm-drive-through-black-ice-lab-example** | **180 s** riddim 160 (`Drive-through - Black Ice - v3`). |
    | **music-edm-drive-through-high-beams-lab-example** | **180 s** color bass 165 (`Drive-through - High Beams - v3`). |
    | **music-edm-drive-through-chain-hook-lab-example** | **180 s** brostep 168 (`Drive-through - Chain Hook - v3`). |
    | **music-edm-drive-through-grit-plate-lab-example** | **180 s** wave bass 150 (`Drive-through - Grit Plate - v3`). |
    | **music-edm-drive-through-steel-grate-lab-example** | **180 s** drumstep 172 (`Drive-through - Steel Grate - v3`). |
    | **music-edm-drive-through-rest-bay-lab-example** | **180 s** chest bass 155 (`Drive-through - Rest Bay - v3`). |
    ...
    | **music-edm-drive-through-spare-drum-lab-example** | **180 s** brostep 165 (`Drive-through - Spare Drum - v3`). |
    | **music-edm-drive-through-oil-pan-lab-example** | **180 s** color bass 150 (`Drive-through - Oil Pan - v3`). |
    | **music-edm-drive-through-curb-check-lab-example** | **180 s** drumstep 174 (`Drive-through - Curb Check - v3`). |
    | **music-edm-drive-through-last-exit-lab-example** | **180 s** dirty bass 160 (`Drive-through - Last Exit - v3`). |
    | **music-edm-drive-through-asphalt-heart-lab-example** | **180 s** chest bass 155 (`Drive-through - Asphalt Heart - v3`). |
    | **music-edm-drive-through-clutch-slam-lab-example** | **180 s** tearout 168 (`Drive-through - Clutch Slam - v3`). |
    | **music-edm-drive-through-trailer-hitch-lab-example** | **180 s** hybrid trap 165 (`Drive-through - Trailer Hitch - v3`). |
    | **music-edm-drive-through-hush-lane-lab-example** | **180 s** dirty dubstep 140 (`Drive-through - Hush Lane - v4`). |
    ...
    | **music-edm-drive-through-fog-vault-lab-example** | **180 s** color bass 142 (`Drive-through - Fog Vault - v4`). |
    | **music-edm-drive-through-dummy-light-lab-example** | **180 s** hybrid trap 145 (`Drive-through - Dummy Light - v4`). |
    ...
    | **music-edm-drive-through-coded-sub-lab-example** | **180 s** chest bass 140 (`Drive-through - Coded Sub - v4`). |
    | **music-edm-drive-through-shadow-coil-lab-example** | **180 s** neuro bass 150 (`Drive-through - Shadow Coil - v4`). |
    | **music-edm-drive-through-mute-pyro-lab-example** | **180 s** wave bass 150 (`Drive-through - Mute Pyro - v4`). |
    ...
    | **music-edm-drive-through-cold-cache-lab-example** | **180 s** hybrid trap 142 (`Drive-through - Cold Cache - v4`). |
    | **music-edm-drive-through-secret-homage-lab-example** | **180 s** dirty dubstep 140 (`Drive-through - Secret Homage - v4`). |
