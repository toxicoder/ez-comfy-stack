...
7. Daily still / GIF / IG pack: **klein-still-daily-lab-example** → optional **wan-gif-loop-lab-example** (LoadImage = `ez_still_app_*.png`, leave ping-pong on) or **klein-dream-house-lab-example** for a 10-photo virtual tour of one place (type any place; outside, rooms, terrace, drone). Character loop: **klein-character-draft-lab-example** → **klein-character-tweak-lab-example** (`ez_character_*.png`).
