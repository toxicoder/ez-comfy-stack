...
- Cascade: ingest -> ASR turns -> speaker cluster (`ve.pt`) -> merge -> translate -> clone -> duration lock (\<=1.25x, then spill, then fade-trim)
...
| Clone | Chatterbox Multilingual V3 (cached `from_local` + ISO `language_id`, PerTh on). Cross-lang `cfg_weight` auto is **0.3** (retry 0.5). `repetition_penalty` 1.9 and `top_p` 1.0 (anti-loop: lower penalties let T3 loop tokens, which HiFT vocodes as a low F0-glide moan with no words). `EZ_DUB_TUNE_CLONE_TOP_P`, `EZ_DUB_TUNE_TEMPERATURE`, `EZ_DUB_TUNE_REPETITION_PENALTY`, `EZ_DUB_TUNE_MIN_P`, `EZ_DUB_TUNE_CFG_CROSS`, `EZ_DUB_TUNE_EXAGGERATION`, and `EZ_DUB_TUNE_REF_*` override those knobs without a code change; a bad value is ignored and logged once. Voice conditionals are prepared once per speaker reference. Lines over 300 characters split; each chunk is onset-cropped then crossfaded. T3 is capped per line (floor 200 tokens so hush cannot eat the budget). Duration lock: ffmpeg `atempo` \<=1.25x at default Speaking speed 1.0, then spill, then fade-trim the end. First Queue is offline when the clone pack is complete (`Cangjie5_TC.json` + `MODELS_DIR/pkuseg`). ~40 s/turn on GB10 is sequential sampling, not a leftover download | MIT | `download-dub --tier clone` |
...
| **Speaking speed** | 1.0 default uses the lab **1.25x** pitch-preserving lock (then spill, then fade-trim the end). 1.5 still caps at 1.25. Not a Kokoro/TTS rate |
...
sequenceDiagram
  ...
  R->>R: scored 4-10 s refs + cached Chatterbox V3 clone (CFG auto)
