...
| Clone | Chatterbox Multilingual V3 (cached `from_local` + ISO `language_id`, PerTh on). Cross-lang `cfg_weight` auto is **0.3** (retry 0.5). `repetition_penalty` 1.2 (V3 `generate` default). Lines over 300 characters split; each chunk is onset-cropped then crossfaded. T3 is capped per line (floor 200 tokens so hush cannot eat the budget). Duration lock: ffmpeg `atempo` ≤1.25× at default Speaking speed 1.0, then spill, then fade-trim the end. First Queue is offline when the clone pack is complete (`Cangjie5_TC.json` + `MODELS_DIR/pkuseg`). ~40 s/turn on GB10 is sequential sampling, not a leftover download | MIT | `download-dub --tier clone` |
