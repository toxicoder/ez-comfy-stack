...
**Who this is for:** studio users who loaded `stills/style-lock` from Apps or Workflows.

> Generated from `workflows/_lab/stills/style-lock.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **stills/style-lock** from **Apps** or **Workflows**
...
**This graph:** `## stills/style-lock Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Four Klein 4B stills of **one place** from new cameras (Prompt Join lock=view). Type any...`
