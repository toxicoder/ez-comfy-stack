...
**Who this is for:** studio users who loaded `stills/still-daily` from Apps or Workflows.

> Generated from `workflows/_lab/stills/still-daily.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **stills/still-daily** from **Apps** or **Workflows**
...
**This graph:** `## stills/still-daily Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Daily Klein 4B still app. Click the UNET filename to swap Apache Klein 4B weights. CLIP...`
