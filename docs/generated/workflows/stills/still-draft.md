...
**Who this is for:** studio users who loaded `stills/still-draft` from Apps or Workflows.

> Generated from `workflows/_lab/stills/still-draft.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Queue this first. Then open stills/still-hero with the same seed for 1280x720.
...
**This graph:** `## stills/still-draft Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. First-run Klein 4B still (Apache 2.0 distilled FP8). Models: flux-2-klein-4b-fp8.safete…`
