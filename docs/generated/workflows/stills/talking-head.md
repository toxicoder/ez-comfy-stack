...
**Who this is for:** studio users who loaded `stills/talking-head` from Apps or Workflows.

> Generated from `workflows/_lab/stills/talking-head.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Canvas is 1280x704 (divisible by 32). Lab printers stay 5.00 s.
Prompt enhance is **off** (on-box Qwen3-4B-Instruct-2507). After Queue, the Enhance node shows the prompt CLIP used (or a passthrough reason). Turn Enhance off to use the widget text as-is. Optional style dropdown.
```

## How to Queue

1. `./scripts/manage.sh start` so `_lab` is seeded
2. Load **stills/talking-head** from **Apps** or **Workflows**
...
| 4 | Klein identity still | `LoadImage` | INPUT |
...
| 24 | Upscale still | `EZImageUpscale` | OUTPUT |
| 25 | Describe image | `EZImageDescribe` | INPUT |
| 26 | Check models | `EZModelCheck` | QUALITY |
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `The start image holds as the first frame. The subject holds still and speaks on...` |
...
**This graph:** `ez_talking_head_frames`
...
**This graph:** `## stills/talking-head Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Klein still (LoadImage) -> optional Qwen3-TTS 0.6B (download-podcast --tier qwen3tts) ...`
...
**This graph:** `The start image holds as the first frame. The subject holds still and speaks one short line. Wardrobe and set stay locked. Mouth motion is modest. Room tone sits under the voice. No music and no scor...`

```text
The start image holds as the first frame. The subject holds still and speaks one short line. Wardrobe and set stay locked. Mouth motion is modest. Room tone sits under the voice. No music and no score.
```
...
**This graph:** `stills/talking-head`
