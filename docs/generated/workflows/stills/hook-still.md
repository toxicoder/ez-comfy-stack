...
**Who this is for:** studio users who loaded `stills/hook-still` from Apps or Workflows.

> Generated from `workflows/_lab/stills/hook-still.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **stills/hook-still** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Positive | `A photoreal still vertical hook still for Shorts. First-frame energy: an origin...` |
...
**This graph:** `## stills/hook-still Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. First-frame Shorts hook still (Klein 4B). Default 432x768 (9:16). Prefix `ez_hook_still`...`
...
**This graph:** `A photoreal still vertical hook still for Shorts. First-frame energy: an original techno wizard fills the lower third mid-stride on a tropical rooftop terrace at golden hour. Tight 9:16, caption head...`

```text
A photoreal still vertical hook still for Shorts. First-frame energy: an original techno wizard fills the lower third mid-stride on a tropical rooftop terrace at golden hour. Tight 9:16, caption headroom at the top. Warm gold-cyan holographic glyph rings, empty of lettering.
```
