...
**Who this is for:** studio users who loaded `wan/sticker-loop` from Apps or Workflows.

> Generated from `workflows/_lab/wan/sticker-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **wan/sticker-loop** from **Apps** or **Workflows**
...
**This graph:** `## wan/sticker-loop Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Loopable sticker MP4 (Wan 5B, 49 frames @ 12 fps, ping-pong ON). Prefix `ez_sticker`. Loa…`
