...
**Who this is for:** studio users who loaded `wan/i2v-5s` from Apps or Workflows.

> Generated from `workflows/_lab/wan/i2v-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
This graph is a smoke/demo (121 frames). For 90s films or ez_shot_01..06 concat, use wan/i2v-shot (120 frames + last frame).
...
2. Load **wan/i2v-5s** from **Apps** or **Workflows**
...
**This graph:** `## wan/i2v-5s After Queue, click **Save video (MP4) — open node for preview** for an inline preview. File lands on the host at `${COMFY_OUTPUT_DIR}/ez_*_*.mp4` (container `/outputs`). Save frames PNG…`
