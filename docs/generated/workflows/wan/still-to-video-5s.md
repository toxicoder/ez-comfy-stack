...
**Who this is for:** studio users who loaded `wan/still-to-video-5s` from Apps or Workflows.

> Generated from `workflows/_lab/wan/still-to-video-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Wan 2.2 TI2V-5B Apache 2.0 silent I2V smoke (~5 s @ 24 fps).
...
LoadImage default example.png. After a still, set ez_still_draft_*.png or ez_still_hero_*.png.
Edit Motion / prompt only. PRIMARY OUTPUT: VHS_VideoCombine MP4 24 fps.

This graph is a smoke/demo (121 frames). For 90s films or ez_shot_01..06 concat, use wan/still-to-shot (120 frames + last frame).
...
2. Load **wan/still-to-video-5s** from **Apps** or **Workflows**
...
**This graph:** `## wan/still-to-video-5s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. After Queue, click **Save video (MP4) — open node for preview** for an inline previe…`
...
**This graph:** `wan/still-to-video-5s`
