...
**Who this is for:** studio users who loaded `wan/parallax-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/wan/parallax-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **wan/parallax-still-5s** from **Apps** or **Workflows**
...
**This graph:** `## wan/parallax-still-5s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Silent Wan 5B I2V subtle parallax (~5 s). LoadImage: a still. Prefix `ez_parallax_vi…`
...
**This graph:** `wan/parallax-still-5s`
