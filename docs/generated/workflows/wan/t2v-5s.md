...
**Who this is for:** studio users who loaded `wan/t2v-5s` from Apps or Workflows.

> Generated from `workflows/_lab/wan/t2v-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
No native audio. For I2V from a still, use wan/i2v-5s.
...
2. Load **wan/t2v-5s** from **Apps** or **Workflows**
...
**This graph:** `## wan/t2v-5s After Queue, click **Save video (MP4) — open node for preview** for an inline preview. File lands on the host at `${COMFY_OUTPUT_DIR}/ez_*_*.mp4` (container `/outputs`). Save frames PNG…`
