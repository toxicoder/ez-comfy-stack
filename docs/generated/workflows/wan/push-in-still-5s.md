...
**Who this is for:** studio users who loaded `wan/push-in-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/wan/push-in-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **wan/push-in-still-5s** from **Apps** or **Workflows**
...
**This graph:** `## wan/push-in-still-5s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Silent Wan 5B I2V hero push-in (~5 s). LoadImage: a still or thumbnail. Prefix `ez_pu…`
...
**This graph:** `wan/push-in-still-5s`
