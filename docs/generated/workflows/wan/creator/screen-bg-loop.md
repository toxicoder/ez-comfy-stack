---
title: wan/creator/screen-bg-loop
...
**Who this is for:** studio users who loaded `wan/creator/screen-bg-loop` from Apps or Workflows.

> Generated from `workflows/_lab/wan/creator/screen-bg-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent desk/screen loop. Lab size **832×480**. Prefix `ez_screen`.
...
2. Load **wan/creator/screen-bg-loop** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Soft screen glow and dust motes. Locked camera. Ping-pong friendly. Keep every …` |
...
**This graph:** `## wan/creator/screen-bg-loop Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Silent desk/screen loop. Lab size **832×480**. Prefix `ez_screen`. Empty of let…`
...
**This graph:** `Soft screen glow and dust motes. Locked camera. Ping-pong friendly. Keep every object and surface from the start image; do not redesign.`

```text
Soft screen glow and dust motes. Locked camera. Ping-pong friendly. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `wan/creator/screen-bg-loop`
