...
**Who this is for:** studio users who loaded `wan/creator/candle-loop` from Apps or Workflows.

> Generated from `workflows/_lab/wan/creator/candle-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent candle-flicker loop. Lab size **832×480**. Prefix `ez_candle`.
...
2. Load **wan/creator/candle-loop** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `A candle flame loops. Locked camera. Ping-pong friendly. Keep every object and …` |
...
**This graph:** `## wan/creator/candle-loop Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Silent candle-flicker loop. Lab size **832×480**. Prefix `ez_candle`. Empty of let…`
...
**This graph:** `A candle flame loops. Locked camera. Ping-pong friendly. Keep every object and surface from the start image; do not redesign.`

```text
A candle flame loops. Locked camera. Ping-pong friendly. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `wan/creator/candle-loop`
