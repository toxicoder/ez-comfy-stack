---
title: wan/creator/light-leak-loop
...
**Who this is for:** studio users who loaded `wan/creator/light-leak-loop` from Apps or Workflows.

> Generated from `workflows/_lab/wan/creator/light-leak-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent light-leak overlay loop. Lab size **832×480**. Prefix `ez_leak`.
...
2. Load **wan/creator/light-leak-loop** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Warm light leak drifts across the frame. Ping-pong friendly. No subject walk. K…` |
...
**This graph:** `## wan/creator/light-leak-loop Silent light-leak overlay loop. Lab size **832×480**. Prefix `ez_leak`. Empty of lettering. Add titles in your editor, not in the prompt. Occupancy: wan — stop LTX, pod…`
...
**This graph:** `Warm light leak drifts across the frame. Ping-pong friendly. No subject walk. Keep every object and surface from the start image; do not redesign.`

```text
Warm light leak drifts across the frame. Ping-pong friendly. No subject walk. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `wan/creator/light-leak-loop`
