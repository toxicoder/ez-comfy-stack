---
title: wan/creator/spotify-canvas
...
**Who this is for:** studio users who loaded `wan/creator/spotify-canvas` from Apps or Workflows.

> Generated from `workflows/_lab/wan/creator/spotify-canvas.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent Spotify Canvas rebound 9:16. Lab size **480×832**. Prefix `ez_spot_canvas`.
...
Spotify Canvas is silent 9:16, 3–8 s, MP4 or JPG. Scale in an editor if the current Canvas pixel range rejects this height.
...
2. Load **wan/creator/spotify-canvas** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Seamless rebound loop. No talking, no singing, no lettering. Locked camera. Kee…` |
...
**This graph:** `ez_spot_canvas_frames`
...
**This graph:** `## wan/creator/spotify-canvas Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Silent Spotify Canvas rebound 9:16. Lab size **480×832**. Prefix `ez_spot_canva…`
...
**This graph:** `Seamless rebound loop. No talking, no singing, no lettering. Locked camera. Keep every object and surface from the start image; do not redesign.`

```text
Seamless rebound loop. No talking, no singing, no lettering. Locked camera. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `wan/creator/spotify-canvas`
