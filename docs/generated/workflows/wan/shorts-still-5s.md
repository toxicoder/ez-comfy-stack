...
**Who this is for:** studio users who loaded `wan/shorts-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/wan/shorts-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **wan/shorts-still-5s** from **Apps** or **Workflows**
...
| 8 | Vertical size + length (9:16) | `Wan22ImageToVideoLatent` | Ungrouped |
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Locked vertical framing for Shorts. Gentle subject motion. Camera holds, then a…` |
...
**This graph:** `ez_shorts_wan_video`
...
**This graph:** `## wan/shorts-still-5s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Vertical silent Wan 5B I2V for Shorts (~5 s @ 24 fps). Apache 2.0. LoadImage: `ez_shor…`
...
**This graph:** `Locked vertical framing for Shorts. Gentle subject motion. Camera holds, then a slow push-in. Keep the start-image identity locked. Keep every object and surface from the start image; do not redesign…`

```text
Locked vertical framing for Shorts. Gentle subject motion. Camera holds, then a slow push-in. Keep the start-image identity locked. Keep every object and surface from the start image; do not redesign. One continuous ~5 s take at 24 fps. No audio.
```
...
**This graph:** `wan/shorts-still-5s`
