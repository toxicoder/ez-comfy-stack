---
title: "services/real-estate/key-turn-loop"
description: "Silent key-turn loop"
tags: [workflows, generated, comfyui, services]
---

# services/real-estate/key-turn-loop
...
**Who this is for:** studio users who loaded `services/real-estate/key-turn-loop` from Apps or Workflows.

> Generated from `workflows/_lab/services/real-estate/key-turn-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent key-turn loop. Lab size **768x768**. Prefix `ez_svc_key`.
...
2. Load **services/real-estate/key-turn-loop** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `A key turns in an unmarked lock, then settles. Locked camera. Ping-pong friendl...` |
...
**This graph:** `## services/real-estate/key-turn-loop Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent key-turn loop. Lab size **768x768**. Prefix `ez_svc_key`. Empty ...`
...
**This graph:** `A key turns in an unmarked lock, then settles. Locked camera. Ping-pong friendly. No walk. Keep every object and surface from the start image; do not redesign.`

```text
A key turns in an unmarked lock, then settles. Locked camera. Ping-pong friendly. No walk. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `services/real-estate/key-turn-loop`
