...
**Who this is for:** studio users who loaded `optional/trellis2` from Apps or Workflows.

> Generated from `workflows/_lab/optional/trellis2.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **optional/trellis2** from **Apps** or **Workflows**
...
| 21 | Save GLB | `MeshToFile3D` | OUTPUT |
...
**This graph:** `## optional/trellis2 Klein still → native TRELLIS.2 INT8 mesh (Comfy core nodes). ```bash ./scripts/manage.sh download-3d --tier trellis2 ./scripts/manage.sh occupancy enter trellis --yes ``` LoadIma…`
