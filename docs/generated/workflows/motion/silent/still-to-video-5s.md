...
**Who this is for:** studio users who loaded `motion/silent/still-to-video-5s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/still-to-video-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
This graph is a smoke/demo (121 frames). For 90s films or ez_shot_01..06 concat, use motion/silent/still-to-shot (120 frames + last frame).
...
2. Load **motion/silent/still-to-video-5s** from **Apps** or **Workflows**
...
**This graph:** `## motion/silent/still-to-video-5s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. After Queue, click **Save video (MP4) — open node for preview** for an inl…`
...
**This graph:** `motion/silent/still-to-video-5s`
