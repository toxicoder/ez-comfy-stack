...
**Who this is for:** studio users who loaded `motion/silent/first-last-5s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/first-last-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/silent/first-last-5s** from **Apps** or **Workflows**
...
| 14 | Operator note - Fun InP FLF | `Note` | NOTE |
| 15 | Wan Prompt Enhance | `EZWanPromptEnhance` | PROMPT |
| 16 | End frame (Fun InP) | `LoadImage` | INPUT |
| 17 | Negative Prompt Enhance | `EZNegativePromptEnhance` | PROMPT |
| 18 | Quality | `EZQuality` | QUALITY |
| 19 | Format / platform | `EZVideoFormat` | SETTINGS |
| 20 | Describe image | `EZImageDescribe` | INPUT |
| 21 | Check models | `EZModelCheck` | QUALITY |
...
**This graph:** `## Fun InP FLF (opt-in) Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. ./scripts/utilities/download-wan.sh run --tier fun-inp Apache 2.0. Not part of downlo...`
...
**This graph:** `motion/silent/first-last-5s`
