...
**Who this is for:** studio users who loaded `motion/silent/orbit-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/orbit-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/silent/orbit-still-5s** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Slow orbit around the start-image subject. Camera arcs a few degrees right whil...` |
...
**This graph:** `## motion/silent/orbit-still-5s Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent Wan 5B I2V slow orbit (~5 s). LoadImage: packshot or still (`ez_packsh...`
...
**This graph:** `Slow orbit around the start-image subject. Camera arcs a few degrees right while keeping the product or hero identity locked. Keep every object and surface from the start image; do not redesign. One ...`

```text
Slow orbit around the start-image subject. Camera arcs a few degrees right while keeping the product or hero identity locked. Keep every object and surface from the start image; do not redesign. One continuous ~5 s take, no cuts.
```
...
**This graph:** `motion/silent/orbit-still-5s`
