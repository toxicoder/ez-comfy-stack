...
**Who this is for:** studio users who loaded `motion/silent/parallax-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/parallax-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/silent/parallax-still-5s** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Subtle parallax from the start still. Foreground drifts a hair left, background...` |
...
**This graph:** `## motion/silent/parallax-still-5s Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent Wan 5B I2V subtle parallax (~5 s). LoadImage: a still. Prefix `ez_p...`
...
**This graph:** `Subtle parallax from the start still. Foreground drifts a hair left, background holds. Locked framing, Ken Burns-like depth, identity locked. One continuous ~5 s take. Keep every object and surface f...`

```text
Subtle parallax from the start still. Foreground drifts a hair left, background holds. Locked framing, Ken Burns-like depth, identity locked. One continuous ~5 s take. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `motion/silent/parallax-still-5s`
