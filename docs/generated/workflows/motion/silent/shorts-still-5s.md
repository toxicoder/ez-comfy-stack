...
**Who this is for:** studio users who loaded `motion/silent/shorts-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/shorts-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/silent/shorts-still-5s** from **Apps** or **Workflows**
...
| 8 | Vertical size + length (9:16) | `Wan22ImageToVideoLatent` | SETTINGS |
...
**This graph:** `## motion/silent/shorts-still-5s Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Vertical silent Wan 5B I2V for Shorts (~5 s @ 24 fps). Apache 2.0. LoadImage...`
...
**This graph:** `motion/silent/shorts-still-5s`
