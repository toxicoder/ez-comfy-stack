...
**Who this is for:** studio users who loaded `motion/silent/push-in-still-5s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/push-in-still-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/silent/push-in-still-5s** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Slow cinematic push-in toward the start-image subject. Keep identity locked. On...` |
...
**This graph:** `## motion/silent/push-in-still-5s Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent Wan 5B I2V hero push-in (~5 s). LoadImage: a still or thumbnail. Pre...`
...
**This graph:** `Slow cinematic push-in toward the start-image subject. Keep identity locked. One continuous ~5 s take at 24 fps. No orbit, no cut. Keep every object and surface from the start image; do not redesign.`

```text
Slow cinematic push-in toward the start-image subject. Keep identity locked. One continuous ~5 s take at 24 fps. No orbit, no cut. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `motion/silent/push-in-still-5s`
