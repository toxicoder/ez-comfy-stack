...
**Who this is for:** studio users who loaded `motion/silent/vace-join` from Apps or Workflows.

> Generated from `workflows/_lab/motion/silent/vace-join.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Opt-in Wan 2.1 VACE 1.3B Apache join smoke. Not download-models.
Download: ./scripts/utilities/download-wan.sh run --tier vace
Unload LTX first. MagCache is off (never on this graph).
Frame count is 17 (1+8n, inside 16-24). Even latents crash.
Models: diffusion_pytorch_model.safetensors (VACE 1.3B) + umt5_xxl_fp8_e4m3fn_scaled.safetensors (CLIP type wan) + wan2.2_vae.safetensors.
Load Shot A last frame and Shot B first frame. Native WanVaceToVideo is the intended join when the Comfy pin exposes it; this graph Queues on core Wan22ImageToVideoLatent so the canvas loads without a missing-node error.
PRIMARY OUTPUT: VHS_VideoCombine MP4 24 fps. Prefix ez_vace_join.
Do not coreside with TRELLIS, Fun InP, SeedVR2, or LTX.
...
2. Load **motion/silent/vace-join** from **Apps** or **Workflows**
...
```mermaid
flowchart LR
  N1["Wan 2.1 VACE 1.3B"]
  N2["UMT5 CLIP (wan)"]
  N3["Wan 2.2 VAE"]
  N4["Shot A last frame"]
  N6["Motion / prompt"]
  N7["Negative"]
  N8["Join length 17 (1+8n)"]
  N9["Shift"]
  N10["KSampler"]
  N11["VAE Decode"]
...
| Id | Title | Type | Group |
| --- | --- | --- | --- |
| 1 | Wan 2.1 VACE 1.3B | `UNETLoader` | Load models |
| 2 | UMT5 CLIP (wan) | `CLIPLoader` | Load models |
| 3 | Wan 2.2 VAE | `VAELoader` | Load models |
| 4 | Shot A last frame | `LoadImage` | For i2v, LoadImage enabled (byp... |
| 6 | Motion / prompt | `CLIPTextEncode` | PROMPT |
| 7 | Negative | `CLIPTextEncode` | PROMPT |
| 8 | Join length 17 (1+8n) | `Wan22ImageToVideoLatent` | SETTINGS |
...
| 16 | Shot B first frame (join) | `LoadImage` | INPUT |
| 17 | Negative Prompt Enhance | `EZNegativePromptEnhance` | PROMPT |
| 18 | Quality | `EZQuality` | QUALITY |
...
**This graph:** `diffusion_pytorch_model.safetensors`
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Hold identity from Shot A last frame and travel toward Shot B first frame. One ...` |
...
**This graph:** `ez_vace_join_frames`
...
**This graph:** `## motion/silent/vace-join Opt-in Wan 2.1 VACE 1.3B Apache join smoke. Not download-models. Download: ./scripts/utilities/download-wan.sh run --tier vace Unload LTX first. MagCache is off (never on t...`
...
**This graph:** `Hold identity from Shot A last frame and travel toward Shot B first frame. One continuous 17-frame join at 24 fps. Keep wardrobe and set locked. Do not redesign.`

```text
Hold identity from Shot A last frame and travel toward Shot B first frame. One continuous 17-frame join at 24 fps. Keep wardrobe and set locked. Do not redesign.
```
...
**This graph:** `motion/silent/vace-join`
