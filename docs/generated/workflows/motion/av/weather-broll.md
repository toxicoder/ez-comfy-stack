...
**Who this is for:** studio users who loaded `motion/av/weather-broll` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/weather-broll.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Weather B-roll AV (~12 s T2V). LTX-2.5 distilled. Community License — not Apache.
...
2. Load **motion/av/weather-broll** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Positive | `Locked-camera weather B-roll. A tropical storm streaks across a rooftop terrace…` |
...
**This graph:** `## motion/av/weather-broll Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invali…`
...
**This graph:** `Locked-camera weather B-roll. A tropical storm streaks across a rooftop terrace and a dark indigo-violet suede running coat; palms thrash and rain sheets off unmarked glass. Soft wind, rain on glass,…`

```text
Locked-camera weather B-roll. A tropical storm streaks across a rooftop terrace and a dark indigo-violet suede running coat; palms thrash and rain sheets off unmarked glass. Soft wind, rain on glass, thunder far off. Unmarked surfaces. No music and no score. Twelve seconds.
```
...
**This graph:** `tropical rain, thunder far off, water on glass, palm slap, no score`
...
**This graph:** `motion/av/weather-broll`
