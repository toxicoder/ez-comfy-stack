...
Weather B-roll AV (~8 s T2V). LTX-2.5 distilled. Community License - not Apache.
...
**This graph:** `## motion/av/weather-broll Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invali...`
...
```text
Locked-camera weather B-roll. A tropical storm streaks across a rooftop terrace and a dark indigo-violet suede running coat; palms thrash and rain sheets off unmarked glass. Soft wind, rain on glass, thunder far off. Unmarked surfaces. No music and no score. Eight seconds.
```
