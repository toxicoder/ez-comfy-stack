...
**Who this is for:** studio users who loaded `motion/av/still-to-shot` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/still-to-shot.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
US-safe 5.00s AV I2V print (121 frames = 1+8n @ 24 fps) with last-frame SaveImage.
Models: ltx-2.5-22b-distilled-transformer-comfy-int8-convrot + gemma4-12b-with-proj-ltx-2.5-comfy-int8-convrot (CLIP ltxv) + ltx-2.5-video-vae-bf16 + ltx-2.5-audio-vae-bf16.
...
90s films: prefer the one-click **films/go-see** graphs (Queue once). This graph is the generic 5.00s printer / spark-farm shot. Paste an `ltx_i2v` paragraph from `{film}.shots.yaml` if you iterate a single beat. VHS prefix ez_<slug>_bN_sM_ltx_video. Host stitch: ./scripts/utilities/concat-shots.sh --film go-see --yes

Do not Queue a 90s denoise. US-safe local pack only. No score.
90s films: prefer the one-click **films/go-see** graphs (Queue once). This graph remains the generic 5.00s printer / spark-farm shot.
...
2. Load **motion/av/still-to-shot** from **Apps** or **Workflows**
...
| 23 | Negative Prompt Enhance | `EZNegativePromptEnhance` | PROMPT |
| 24 | Quality | `EZQuality` | QUALITY |
| 25 | Format / platform | `EZVideoFormat` | SETTINGS |
...
| Instance | Value |
| --- | --- |
| Motion + audio | `The start image holds as the first frame. Wordless mix: silent mouth. Race-pace...` |
...
**This graph:** `## motion/av/still-to-shot Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invali...`
...
**This graph:** `ez_gosee_b1_s1_ltx_video`
...
**This graph:** `The start image holds as the first frame. Wordless mix: silent mouth. Race-pace footfalls strike terrace stone as the dead sprint continues, warm wind and grit ticks with each stride, close-mic breat...`

```text
The start image holds as the first frame. Wordless mix: silent mouth. Race-pace footfalls strike terrace stone as the dead sprint continues, warm wind and grit ticks with each stride, close-mic breath on the contralateral pump. First-person eye-level body-cam tracks forward, horizon bobbing, no cut. The wearer's own ink-black sleeves and blank matte-black gloves stay along the bottom edge, empty palms. The rooftop gap already in the start frame grows dead ahead. Last frames hold the gap in the center until it fills the frame. No speech. No music and no score.
```
...
**This graph:** `motion/av/still-to-shot`
