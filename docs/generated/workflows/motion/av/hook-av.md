...
**Who this is for:** studio users who loaded `motion/av/hook-av` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/hook-av.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
~8 s AV cold-open / hook (LTX-2.5 T2V). Community License - not Apache.
...
2. Load **motion/av/hook-av** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Positive | `A five-second AV cold open. Camera snaps to the start-image subject with fast p...` |
...
**This graph:** `## motion/av/hook-av Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invalid). ~8...`
...
**This graph:** `A five-second AV cold open. Camera snaps to the start-image subject with fast present-tense energy. World SFX matching the start image. Unmarked surfaces. No music and no score.`

```text
A five-second AV cold open. Camera snaps to the start-image subject with fast present-tense energy. World SFX matching the start image. Unmarked surfaces. No music and no score.
```
