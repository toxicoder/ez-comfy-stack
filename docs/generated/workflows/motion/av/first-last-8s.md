...
**Who this is for:** studio users who loaded `motion/av/first-last-8s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/first-last-8s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
LTX-2.5 distilled **first-last-frame** AV (~8 s). Two Klein stills (hero 1280x704) pin start and end via `LTXVAddGuide` on the **video** latent, then audio concat. LTX Community License - not Apache. $10M company-revenue cap. Disclose AI-generated media; do not strip provenance; do not distill.
...
2. Load **motion/av/first-last-8s** from **Apps** or **Workflows**
...
| 20 | First frame | `LoadImage` | INPUT |
| 21 | Last frame | `LoadImage` | INPUT |
| 22 | Guide first frame | `LTXVAddGuide` | SETTINGS |
| 23 | Guide last frame | `LTXVAddGuide` | SETTINGS |
| 24 | Crop guide frames | `LTXVCropGuides` | SETTINGS |
| 25 | Quality | `EZQuality` | QUALITY |
| 26 | Format / platform | `EZVideoFormat` | SETTINGS |
...
**This graph:** `## motion/av/first-last-8s Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invali...`
...
**This graph:** `motion/av/first-last-8s`
...
!!! warning "Lab notes"
    ...
    motion/av/first-last-8s uses index 0 then -1 on the video latent before audio concat.
