...
**Who this is for:** studio users who loaded `motion/av/broll-ambient` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/broll-ambient.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Ambient B-roll AV plate (~8 s T2V). LTX-2.5 distilled. Community License - not Apache.
...
2. Load **motion/av/broll-ambient** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Positive | `Locked-camera ambient B-roll of a tropical coastal city rooftop terrace at gold...` |
...
**This graph:** `## motion/av/broll-ambient Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invali...`
...
```text
Locked-camera ambient B-roll of a tropical coastal city rooftop terrace at golden hour. Coat hem stirs, holographic glyph motes drift, palm fronds move, distant bay traffic and a warm breeze, a glyph chime once. photoreal shot, unmarked surfaces, empty of lettering. No music and no score. Eight seconds.
```
...
**This graph:** `8 seconds, 24 fps, locked camera B-roll`
...
**This graph:** `warm terrace breeze, palm rustle, distant bay traffic, glyph chime, no score`

```text
warm terrace breeze, palm rustle, distant bay traffic, glyph chime, no score
```
...
**This graph:** `motion/av/broll-ambient`
