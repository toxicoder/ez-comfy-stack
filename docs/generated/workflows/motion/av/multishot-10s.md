...
**Who this is for:** studio users who loaded `motion/av/multishot-10s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/multishot-10s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
LTX-2.5 distilled **native multishot** T2V (~10 s). Named cuts in one generation. LTX Community License — not Apache. $10M company-revenue cap. Disclose AI-generated media; do not strip provenance; do not distill.
...
241 frames @ 24 fps. Prompt names a hard cut and a match cut; audio continuity is in the paragraph. Prompt enhance is **off** so authored text is encoded as written. Turn Enhance on only if you want the 4B rewriter.
...
2. Load **motion/av/multishot-10s** from **Apps** or **Workflows**
...
**This graph:** `## motion/av/multishot-10s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invali…`
...
**This graph:** `motion/av/multishot-10s`
