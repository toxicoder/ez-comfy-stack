...
**Who this is for:** studio users who loaded `motion/av/dialogue-8s` from Apps or Workflows.

> Generated from `workflows/_lab/motion/av/dialogue-8s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
LTX-2.5 distilled **dialogue** T2V (~8 s). Joint AV speech Wan cannot mux. LTX Community License - not Apache. $10M company-revenue cap. Disclose AI-generated media; do not strip provenance; do not distill.
...
193 frames @ 24 fps. Authored quoted line; mouths will not match. Prompt enhance is **off** so authored text is encoded as written. Turn Enhance on only if you want the 4B rewriter.
...
2. Load **motion/av/dialogue-8s** from **Apps** or **Workflows**
...
| 20 | LTX A/V coupling | `LTXVModalityGuidance` | SETTINGS |
| 21 | Quality | `EZQuality` | QUALITY |
| 22 | Format / platform | `EZVideoFormat` | SETTINGS |
| 23 | Check models | `EZModelCheck` | QUALITY |
...
**This graph:** `## motion/av/dialogue-8s Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invalid)...`
...
**This graph:** `motion/av/dialogue-8s`
...
!!! warning "Lab notes"
    ...
    motion/av/dialogue-8s uses 3.0 / 0 / 1. Mouths still will not lip-sync; this only tightens A/V coupling.
