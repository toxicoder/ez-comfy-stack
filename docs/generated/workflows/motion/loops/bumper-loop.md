...
**Who this is for:** studio users who loaded `motion/loops/bumper-loop` from Apps or Workflows.

> Generated from `workflows/_lab/motion/loops/bumper-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/loops/bumper-loop** from **Apps** or **Workflows**
...
flowchart LR
  ...
  N6["Motion / prompt"]
  N7["Negative"]
  N8["GIF size and length (49 frames)"]
  N9["Shift"]
  N10["KSampler"]
  N11["VAE Decode"]
...
| 8 | GIF size and length (49 frames) | `Wan22ImageToVideoLatent` | SETTINGS |
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Locked camera bumper loop. Soft cyclic breeze on leaves and fabric. Motion retu...` |
...
**This graph:** `## motion/loops/bumper-loop Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Short loopable MP4 bumper (Wan 5B, 49 frames @ 12 fps, ping-pong ON). Prefix `ez_...`
...
**This graph:** `Locked camera bumper loop. Soft cyclic breeze on leaves and fabric. Motion returns near the start pose for seamless ping-pong. Keep start-image identity locked. No walk, no dolly. Keep every object a...`

```text
Locked camera bumper loop. Soft cyclic breeze on leaves and fabric. Motion returns near the start pose for seamless ping-pong. Keep start-image identity locked. No walk, no dolly. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `looping bumper, 12 fps`
...
**This graph:** `motion/loops/bumper-loop`
