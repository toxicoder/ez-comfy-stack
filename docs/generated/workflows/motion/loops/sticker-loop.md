...
**Who this is for:** studio users who loaded `motion/loops/sticker-loop` from Apps or Workflows.

> Generated from `workflows/_lab/motion/loops/sticker-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **motion/loops/sticker-loop** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Tight looping sticker motion. A small cyclic bounce or shimmer on the subject. ...` |
...
**This graph:** `## motion/loops/sticker-loop Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Loopable sticker MP4 (Wan 5B, 49 frames @ 12 fps, ping-pong ON). Prefix `ez_stic...`
...
**This graph:** `Tight looping sticker motion. A small cyclic bounce or shimmer on the subject. Ping-pong friendly. Keep start-image identity locked. No walk, no dolly. Keep every object and surface from the start im...`

```text
Tight looping sticker motion. A small cyclic bounce or shimmer on the subject. Ping-pong friendly. Keep start-image identity locked. No walk, no dolly. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `looping sticker, 12 fps`
...
**This graph:** `motion/loops/sticker-loop`
