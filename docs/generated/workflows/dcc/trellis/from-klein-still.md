---
title: dcc/trellis/from-klein-still
...
**Who this is for:** studio users who loaded `dcc/trellis/from-klein-still` from Apps or Workflows.

> Generated from `workflows/_lab/dcc/trellis/from-klein-still.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **dcc/trellis/from-klein-still** from **Apps** or **Workflows**
...
**This graph:** `## dcc/trellis/from-klein-still Still pack plate → native TRELLIS.2 INT8 mesh (Comfy core nodes). Occupancy: **trellis**. ```bash ./scripts/manage.sh download-3d --tier trellis2 ./scripts/manage.sh o…`
