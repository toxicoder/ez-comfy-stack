...
**Who this is for:** studio users who loaded `dcc/klein/canny-hero` from Apps or Workflows.

> Generated from `workflows/_lab/dcc/klein/canny-hero.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **dcc/klein/canny-hero** from **Apps** or **Workflows**
...
flowchart LR
  ...
  N4["Positive"]
  N5["Negative"]
  N6["Latent 1280x704 batch 1"]
  N7["KSampler"]
  N8["VAE Decode"]
  N9["Save canny hero"]
  N10["Operator note"]
  N11["Canny first.png (guide pack)"]
  N12["Klein Prompt Enhance (edit)"]
  N13["Encode canny plate"]
  N14["Positive + canny plate"]
...
| 9 | Save canny hero | `SaveImage` | Ungrouped |
| 10 | Operator note | `Note` | Ungrouped |
| 11 | Canny first.png (guide pack) | `LoadImage` | Ungrouped |
| 12 | Klein Prompt Enhance (edit) | `EZKleinPromptEnhance` | Ungrouped |
| 13 | Encode canny plate | `VAEEncode` | Ungrouped |
| 14 | Positive + canny plate | `ReferenceLatent` | Ungrouped |
...
**This graph:** `## dcc/klein/canny-hero Klein 4B **edit** of a DCC line-art plate (guide pack ``canny/`` first frame, or still ``canny.png``). Enhance **on**. Seed **42**. Size **1280x704**. LoadImage: ``canny.png``…`
...
**This graph:** `Keep the line-art silhouette, camera, and layout from the start image. Finish as a photoreal still inside those edges. Unmarked surfaces. Do not invent new geometry.`
...
**This graph:** `dcc/klein/canny-hero`
