...
**Who this is for:** studio users who loaded `dcc/klein/from-canny` from Apps or Workflows.

> Generated from `workflows/_lab/dcc/klein/from-canny.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Stay on :8188 after a dump: dcc/klein/from-guide-loader / dcc/ltx/iclora-from-guide-loader / dcc/trellis/from-klein-still (EZDCCLoadGuideStill + OccupancyGate). The LoadImage + --install-inputs path on this graph still works.
```

## How to Queue

1. `./scripts/manage.sh start` so `_lab` is seeded
2. Load **dcc/klein/from-canny** from **Apps** or **Workflows**
...
**This graph:** `## dcc/klein/from-canny Klein 4B **edit** of a DCC line-art plate (guide pack ``canny/`` first frame, or still ``canny.png``). Enhance **on**. Seed **42**. Size **1280x704**. LoadImage: ``canny.png``…`
...
**This graph:** `dcc/klein/from-canny`
