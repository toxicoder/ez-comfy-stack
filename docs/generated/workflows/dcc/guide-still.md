...
**Who this is for:** studio users who loaded `dcc/guide-still` from Apps or Workflows.

> Generated from `workflows/_lab/dcc/guide-still.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **dcc/guide-still** from **Apps** or **Workflows**
...
**This graph:** `## dcc/guide-still Klein 4B **edit** of a guide-pack still loaded in-canvas (no LoadImage / --install-inputs). Enhance **on**. Seed **42**. Size **1280x704**. Prefix ``ez_guide_hero``. Occupancy: **k…`
