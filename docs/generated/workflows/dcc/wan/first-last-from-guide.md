...
**Who this is for:** studio users who loaded `dcc/wan/first-last-from-guide` from Apps or Workflows.

> Generated from `workflows/_lab/dcc/wan/first-last-from-guide.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Stay on :8188 after a dump: dcc/klein/guide-still / dcc/ltx/depth-from-loader / dcc/trellis/still-to-mesh (EZDCCLoadGuideStill + OccupancyGate). The LoadImage + --install-inputs path on this graph still works.
```

## How to Queue

1. `./scripts/manage.sh start` so `_lab` is seeded
2. Load **dcc/wan/first-last-from-guide** from **Apps** or **Workflows**
...
| 4 | Guide first.png | `LoadImage` | For i2v, LoadImage enabled (byp… |
| 6 | Motion / prompt | `CLIPTextEncode` | Ungrouped |
| 7 | Negative | `CLIPTextEncode` | Ungrouped |
| 8 | Video size & length | `Wan22ImageToVideoLatent` | Ungrouped |
...
| 16 | Guide last.png | `LoadImage` | Ungrouped |
...
| Instance | Value |
| --- | --- |
| Guide first.png | `first.png` |
| Guide last.png | `last.png` |
...
**This graph:** `## dcc/wan/first-last-from-guide Silent Fun InP first-last-frame from a DCC guide pack. LoadImage ``first.png`` and ``last.png``. MagCache **off**. Opt-in: ./scripts/manage.sh download-wan --tier fun…`
...
```text
Slow dolly in toward the start-image subject. Gentle motion in fabric, hair, or foliage. Keep the start-image identity locked. Keep every object and surface from the start image; do not redesign. One continuous five-second take at 24 fps.
```
...
**This graph:** `dcc/wan/first-last-from-guide`
