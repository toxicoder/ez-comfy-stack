...
**Who this is for:** studio users who loaded `audio/finish` from Apps or Workflows.

> Generated from `workflows/_lab/audio/finish.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **audio/finish** from **Apps** or **Workflows**
...
**This graph:** `## audio/finish Picture-lock stem mix. Occupancy **audio** — stop Klein / Wan / LTX first. ACE-Step score is a later session if `score: acestep-instrumental`. This canvas does not denoise video. Mix …`
