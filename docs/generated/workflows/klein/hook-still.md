...
**Who this is for:** studio users who loaded `klein/hook-still` from Apps or Workflows.

> Generated from `workflows/_lab/klein/hook-still.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/hook-still** from **Apps** or **Workflows**
...
**This graph:** `## klein/hook-still Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. First-frame Shorts hook still (Klein 4B). Default 432×768 (9:16). Prefix `ez_hook_still`.…`
