...
**Who this is for:** studio users who loaded `klein/text-swap` from Apps or Workflows.

> Generated from `workflows/_lab/klein/text-swap.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/text-swap** from **Apps** or **Workflows**
...
**This graph:** `## klein/text-swap Klein 4B **lettering swap**. Load a still that already has type. Type the new lettering (or "Replace SALE with OPEN"). Enhance **text_swap** rewrites a glyph-lock instruction: same…`
