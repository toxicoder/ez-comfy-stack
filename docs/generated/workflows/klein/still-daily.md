...
**Who this is for:** studio users who loaded `klein/still-daily` from Apps or Workflows.

> Generated from `workflows/_lab/klein/still-daily.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/still-daily** from **Apps** or **Workflows**
...
```mermaid
flowchart LR
  N1["Image model — click filename to swap"]
  N2["Qwen3-4B TE"]
  N3["Flux2 VAE"]
  N4["Positive"]
  N5["Negative"]
  N6["Size (width x height x batch)"]
  N7["Sampler (seed / steps / CFG)"]
  N8["VAE Decode"]
  N9["Save PNG"]
  N10["Operator note"]
...
| Id | Title | Type | Group |
| --- | --- | --- | --- |
| 1 | Image model — click filename to swap | `UNETLoader` | MODEL |
...
| 6 | Size (width x height x batch) | `EmptyFlux2LatentImage` | SETTINGS |
| 7 | Sampler (seed / steps / CFG) | `KSampler` | SETTINGS |
...
**This graph:** `## klein/still-daily Daily Klein 4B still app. Click the UNET filename to swap Apache Klein 4B weights. CLIP (qwen_3_4b, type flux2) and flux2-vae stay the same for every Klein 4B UNET. Swap table: -…`
