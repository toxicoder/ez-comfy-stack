...
**Who this is for:** studio users who loaded `klein/before-after` from Apps or Workflows.

> Generated from `workflows/_lab/klein/before-after.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/before-after** from **Apps** or **Workflows**
...
| 13 | Decode BEFORE | `VAEDecode` | SHOT BEFORE |
| 14 | Save BEFORE | `SaveImage` | SHOT BEFORE |
| 16 | SHOT AFTER | `EZPromptJoin` | SHOT AFTER |
| 17 | Positive AFTER | `CLIPTextEncode` | SHOT AFTER |
| 18 | Sampler AFTER | `KSampler` | SHOT AFTER |
...
**This graph:** `A photoreal still of a small kitchen table at first light. One cream ceramic mug with a hairline chip on the rim sits empty on honey-oak beside a dark window. White subway tile, one linen curtain. Co…`

```text
A photoreal still of a small kitchen table at first light. One cream ceramic mug with a hairline chip on the rim sits empty on honey-oak beside a dark window. White subway tile, one linen curtain. Cool blue shadows, unmarked surfaces.
```
...
**This graph:** `## klein/before-after Two Klein 4B stills of one mug. SHOT BEFORE is the identity plate; AFTER Klein-edits from it. Prefixes `ez_before` and `ez_after`. Do not bypass BEFORE on a cold canvas. Prompt …`
...
| Instance | Value |
| --- | --- |
| SHOT BEFORE | `Canonical plate. Empty mug, cool blue window light, same 35mm camera.` |
| SHOT AFTER | `Same kitchen, mug, and camera. Mug filled with coffee, warm morning sun, soft s…` |
...
**This graph (all 2 instances):** `cream ceramic mug with a hairline chip, honey-oak table, white subway tile, linen curtain`
...
**This graph (all 2 instances):** `state`
