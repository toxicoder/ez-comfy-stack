...
**Who this is for:** studio users who loaded `klein/podcast-cover` from Apps or Workflows.

> Generated from `workflows/_lab/klein/podcast-cover.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/podcast-cover** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Positive | `A photoreal square podcast-cover still. A ceramic mug and a simple analog recor…` |
...
**This graph:** `## klein/podcast-cover Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Podcast / playlist cover (Klein 4B). Default 1024×1024. Prefix `ez_podcast`. Swap the …`
...
**This graph:** `A photoreal square podcast-cover still. A ceramic mug and a simple analog recorder sit on a wooden table in warm sidelight. Unmarked, empty of logos and lettering. 1:1.`

```text
A photoreal square podcast-cover still. A ceramic mug and a simple analog recorder sit on a wooden table in warm sidelight. Unmarked, empty of logos and lettering. 1:1.
```
...
**This graph:** `klein/podcast-cover`
...
**This graph:** `Podcast · cover (1024×1024)`
