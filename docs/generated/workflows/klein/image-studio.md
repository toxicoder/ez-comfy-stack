...
**Who this is for:** studio users who loaded `klein/image-studio` from Apps or Workflows.

> Generated from `workflows/_lab/klein/image-studio.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/image-studio** from **Apps** or **Workflows**
...
**This graph:** `## klein/image-studio Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Universal Klein 4B still desk with 100 creator modes (background swap, change text, cha…`
