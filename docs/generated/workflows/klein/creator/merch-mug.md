...
**Who this is for:** studio users who loaded `klein/creator/merch-mug` from Apps or Workflows.

> Generated from `workflows/_lab/klein/creator/merch-mug.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Merch mug mock 1:1. Lab size **1024×1024**. Prefix `ez_mug_merch`.
...
2. Load **klein/creator/merch-mug** from **Apps** or **Workflows**
...
**This graph:** `## klein/creator/merch-mug Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Merch mug mock 1:1. Lab size **1024×1024**. Prefix `ez_mug_merch`. Empty of letter…`
...
**This graph:** `A photoreal still of a cream ceramic mug with a hairline chip on the rim, unmarked, on honey-oak beside a dark window. Soft studio key, gentle contact shadow. Empty of lettering. Photoreal ceramic mu…`

```text
A photoreal still of a cream ceramic mug with a hairline chip on the rim, unmarked, on honey-oak beside a dark window. Soft studio key, gentle contact shadow. Empty of lettering. Photoreal ceramic mug three-quarter view on seamless gray. Unmarked, no logos.
```
...
**This graph:** `Merch · mug (1024×1024)`
