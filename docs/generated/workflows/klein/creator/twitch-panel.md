...
**Who this is for:** studio users who loaded `klein/creator/twitch-panel` from Apps or Workflows.

> Generated from `workflows/_lab/klein/creator/twitch-panel.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Twitch about-panel art. Lab size **768×1024**. Prefix `ez_tw_panel`.
...
2. Load **klein/creator/twitch-panel** from **Apps** or **Workflows**
...
**This graph:** `## klein/creator/twitch-panel Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Twitch about-panel art. Lab size **768×1024**. Prefix `ez_tw_panel`. Empty of l…`
...
**This graph:** `A photoreal still of a dim unmarked streaming desk with teal practicals and a quiet backdrop. Empty of lettering and UI chrome. Tall panel still. Subject in the upper half, quiet lower band. Empty of…`

```text
A photoreal still of a dim unmarked streaming desk with teal practicals and a quiet backdrop. Empty of lettering and UI chrome. Tall panel still. Subject in the upper half, quiet lower band. Empty of lettering.
```
...
**This graph:** `Twitch · panel (768×1024)`
