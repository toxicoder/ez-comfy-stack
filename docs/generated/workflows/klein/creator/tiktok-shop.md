...
**Who this is for:** studio users who loaded `klein/creator/tiktok-shop` from Apps or Workflows.

> Generated from `workflows/_lab/klein/creator/tiktok-shop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
TikTok Shop packshot 1:1. Lab size **1024×1024**. Prefix `ez_tt_shop`.
...
2. Load **klein/creator/tiktok-shop** from **Apps** or **Workflows**
...
**This graph:** `## klein/creator/tiktok-shop Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. TikTok Shop packshot 1:1. Lab size **1024×1024**. Prefix `ez_tt_shop`. Empty of …`
...
**This graph:** `A photoreal still of a cream ceramic mug with a hairline chip on the rim, unmarked, on honey-oak beside a dark window. Soft studio key, gentle contact shadow. Empty of lettering. Clean product on a s…`

```text
A photoreal still of a cream ceramic mug with a hairline chip on the rim, unmarked, on honey-oak beside a dark window. Soft studio key, gentle contact shadow. Empty of lettering. Clean product on a seamless light-gray cyclorama, soft studio key, unmarked.
```
...
**This graph:** `TikTok · Shop (1024×1024)`
