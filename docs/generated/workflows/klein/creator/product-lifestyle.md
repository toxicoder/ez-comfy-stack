...
**Who this is for:** studio users who loaded `klein/creator/product-lifestyle` from Apps or Workflows.

> Generated from `workflows/_lab/klein/creator/product-lifestyle.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
In-use product lifestyle 4:5. Lab size **1024×1280**. Prefix `ez_lifestyle`.
...
2. Load **klein/creator/product-lifestyle** from **Apps** or **Workflows**
...
| 6 | Latent (wired from Format) | `EmptyFlux2LatentImage` | PROMPT |
...
**This graph:** `## klein/creator/product-lifestyle Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. In-use product lifestyle 4:5. Lab size **1024×1280**. Prefix `ez_lifestyle…`
...
**This graph:** `A photoreal still of a cream ceramic mug with a hairline chip on the rim, unmarked, on honey-oak beside a dark window. Soft studio key, gentle contact shadow. Empty of lettering. Lifestyle still of a…`

```text
A photoreal still of a cream ceramic mug with a hairline chip on the rim, unmarked, on honey-oak beside a dark window. Soft studio key, gentle contact shadow. Empty of lettering. Lifestyle still of an unmarked ceramic mug in use on a sunlit table. No cyclorama.
```
...
**This graph:** `Product lifestyle (1024×1280)`
