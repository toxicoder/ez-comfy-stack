...
**Who this is for:** studio users who loaded `klein/creator/twitch-profile` from Apps or Workflows.

> Generated from `workflows/_lab/klein/creator/twitch-profile.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Twitch profile photo, circle-safe. Lab size **768×768**. Prefix `ez_tw_profile`.
...
2. Load **klein/creator/twitch-profile** from **Apps** or **Workflows**
...
**This graph:** `## klein/creator/twitch-profile Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Twitch profile photo, circle-safe. Lab size **768×768**. Prefix `ez_tw_profil…`
...
**This graph:** `A photoreal still of a dim unmarked streaming desk with teal practicals and a quiet backdrop. Empty of lettering and UI chrome. Subject centered in a circular safe area so corners may crop. Empty of …`

```text
A photoreal still of a dim unmarked streaming desk with teal practicals and a quiet backdrop. Empty of lettering and UI chrome. Subject centered in a circular safe area so corners may crop. Empty of lettering.
```
...
**This graph:** `Twitch · profile (768×768)`
