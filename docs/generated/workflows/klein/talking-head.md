...
**Who this is for:** studio users who loaded `klein/talking-head` from Apps or Workflows.

> Generated from `workflows/_lab/klein/talking-head.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **klein/talking-head** from **Apps** or **Workflows**
...
| 4 | Klein identity still | `LoadImage` | Ungrouped |
...
**This graph:** `## klein/talking-head Klein still (LoadImage) → optional Qwen3-TTS 0.6B (download-podcast --tier qwen3tts) → LTX-2.5 A2V freeze. Occupancy: ACE-Step 90s bed then stop/unload then this graph. Real sin…`
