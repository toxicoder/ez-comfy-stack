...
**Who this is for:** studio users who loaded `creator/silent/zoom-punch` from Apps or Workflows.

> Generated from `workflows/_lab/creator/silent/zoom-punch.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **creator/silent/zoom-punch** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Punch-in toward the start-image subject in the first second, then hold. Keep ev...` |
...
**This graph:** `## creator/silent/zoom-punch Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent 9:16 hook zoom punch. Lab size **480x832**. Prefix `ez_punch`. Empty of l...`
...
**This graph:** `Punch-in toward the start-image subject in the first second, then hold. Keep every object and surface from the start image; do not redesign.`

```text
Punch-in toward the start-image subject in the first second, then hold. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `creator/silent/zoom-punch`
