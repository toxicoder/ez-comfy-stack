---
title: "creator/silent/confetti-loop"
description: "Silent celebration loop Prompt enhance is **off** so authored text (recipe, script labels, ACE tags, or film shots) is encoded as written. Turn Enhance on onl"
tags: [workflows, generated, comfyui, creator]
---

# creator/silent/confetti-loop
...
**Who this is for:** studio users who loaded `creator/silent/confetti-loop` from Apps or Workflows.

> Generated from `workflows/_lab/creator/silent/confetti-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent celebration loop. Lab size **832x480**. Prefix `ez_confetti`.
...
2. Load **creator/silent/confetti-loop** from **Apps** or **Workflows**
...
| 8 | Loop size 832x480 x 49 | `Wan22ImageToVideoLatent` | SETTINGS |
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Soft falling paper motes, looping. Ping-pong friendly. No flash. Keep every obj...` |
...
**This graph:** `## creator/silent/confetti-loop Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent celebration loop. Lab size **832x480**. Prefix `ez_confetti`. Empty of...`
...
**This graph:** `Soft falling paper motes, looping. Ping-pong friendly. No flash. Keep every object and surface from the start image; do not redesign.`

```text
Soft falling paper motes, looping. Ping-pong friendly. No flash. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `creator/silent/confetti-loop`
