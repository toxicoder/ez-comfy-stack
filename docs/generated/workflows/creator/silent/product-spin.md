...
**Who this is for:** studio users who loaded `creator/silent/product-spin` from Apps or Workflows.

> Generated from `workflows/_lab/creator/silent/product-spin.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent product orbit ~5 s. Lab size **832x480**. Prefix `ez_spin`.
...
2. Load **creator/silent/product-spin** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Slow orbit around the start-image product. Table holds. No cut. Keep every obje...` |
...
**This graph:** `## creator/silent/product-spin Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent product orbit ~5 s. Lab size **832x480**. Prefix `ez_spin`. Empty of le...`
...
**This graph:** `Slow orbit around the start-image product. Table holds. No cut. Keep every object and surface from the start image; do not redesign.`

```text
Slow orbit around the start-image product. Table holds. No cut. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `creator/silent/product-spin`
