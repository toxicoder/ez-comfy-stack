...
**Who this is for:** studio users who loaded `creator/silent/pan-left` from Apps or Workflows.

> Generated from `workflows/_lab/creator/silent/pan-left.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent locked-identity pan ~5 s. Lab size **832x480**. Prefix `ez_pan`.
...
2. Load **creator/silent/pan-left** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Slow pan left. Keep every object from the start image. No cut. Keep every objec...` |
...
**This graph:** `## creator/silent/pan-left Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent locked-identity pan ~5 s. Lab size **832x480**. Prefix `ez_pan`. Empty of l...`
...
**This graph:** `Slow pan left. Keep every object from the start image. No cut. Keep every object and surface from the start image; do not redesign.`

```text
Slow pan left. Keep every object from the start image. No cut. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `creator/silent/pan-left`
