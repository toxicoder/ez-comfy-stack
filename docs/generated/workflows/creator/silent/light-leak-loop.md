...
# creator/silent/light-leak-loop
...
**Who this is for:** studio users who loaded `creator/silent/light-leak-loop` from Apps or Workflows.

> Generated from `workflows/_lab/creator/silent/light-leak-loop.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Silent light-leak overlay loop. Lab size **832x480**. Prefix `ez_leak`.
...
2. Load **creator/silent/light-leak-loop** from **Apps** or **Workflows**
...
**This graph:** `## creator/silent/light-leak-loop Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. Silent light-leak overlay loop. Lab size **832x480**. Prefix `ez_leak`. Emp...`
...
**This graph:** `creator/silent/light-leak-loop`
