...
**Who this is for:** studio users who loaded `creator/av/tiktok-broll` from Apps or Workflows.

> Generated from `workflows/_lab/creator/av/tiktok-broll.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
TikTok B-roll AV 9:16. Lab size **768x1280**. Prefix `ez_tt_broll`.
...
2. Load **creator/av/tiktok-broll** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Vertical B-roll from the start still. World SFX only. No score. Eight seconds. ...` |
...
**This graph:** `## creator/av/tiktok-broll Format / platform sets pixels (Custom uses Width x Height). Quality does not change size. TikTok B-roll AV 9:16. Lab size **768x1280**. Prefix `ez_tt_broll`. Empty of lette...`
...
**This graph:** `Vertical B-roll from the start still. World SFX only. No score. Eight seconds. Keep every object and surface from the start image; do not redesign.`

```text
Vertical B-roll from the start still. World SFX only. No score. Eight seconds. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `creator/av/tiktok-broll`
