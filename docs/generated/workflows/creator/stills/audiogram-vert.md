...
**Who this is for:** studio users who loaded `creator/stills/audiogram-vert` from Apps or Workflows.

> Generated from `workflows/_lab/creator/stills/audiogram-vert.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **creator/stills/audiogram-vert** from **Apps** or **Workflows**
...
**This graph:** `## creator/stills/audiogram-vert Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Audiogram background 9:16. Lab size **576×1024**. Prefix `ez_ag_vert`. Empty…`
