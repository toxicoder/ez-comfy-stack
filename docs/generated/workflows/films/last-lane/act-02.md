...
This graph is **act 2/5** of a 7.5 min film (90 × 5.00s). Act 2 LoadImage should be the previous act last frame (ez_lastlane_b6_s3_last_*.png). In-graph stitch writes `ez_lastlane_act2_90s.mp4`. After all five acts: `concat-shots.sh --film last-lane --yes` → `ez_lastlane_450s.mp4`.
...
**This graph:** `## last-lane (night two-lane · Fork and wash) Queue **once**. Klein identity still → 18 × 5.00s LTX AV prints (last-frame continuity) → **Save 90s film (MP4)**. Do not Queue a 90s denoise (121 frames…`

```text
## last-lane (night two-lane · Fork and wash)

Queue **once**. Klein identity still → 18 × 5.00s LTX AV prints (last-frame continuity) → **Save 90s film (MP4)**. Do not Queue a 90s denoise (121 frames = 1+8n per shot). Optional silent rehearsal: **motion/silent/still-to-shot**. Optional host stitch: `./scripts/utilities/concat-shots.sh --film last-lane --yes`.
