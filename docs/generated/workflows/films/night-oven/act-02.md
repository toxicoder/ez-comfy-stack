...
**Who this is for:** studio users who loaded `films/night-oven/act-02` from Apps or Workflows.

> Generated from `workflows/_lab/films/night-oven/act-02.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
This graph is **act 2/5** of a 7.5 min film (90 x 5.00s). Act 2 LoadImage should be the previous act last frame (ez_nightoven_b6_s3_last_*.png). In-graph stitch writes `ez_nightoven_act2_90s.mp4`. After all five acts: `concat-shots.sh --film night-oven --yes` -> `ez_nightoven_450s.mp4`.
...
2. Load **films/night-oven/act-02** from **Apps** or **Workflows**
...
**This graph:** `## films/night-oven/act-02 LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invalid). The stitched MP4 is written automatically to `${COMFY_OUTPUT_DIR}` (container `/output...`
...
**This graph:** `films/night-oven/act-02`
...
**This graph:** `## night-oven (overnight bakery - Heat) Queue **once**. Klein identity still -> 18 x 5.00s LTX AV prints (last-frame continuity) -> **Save 90s film (MP4)**. Do not Queue a 90s denoise (121 frames = 1...`
...
**This graph (all 18 instances):** `films/night-oven/act-02`
