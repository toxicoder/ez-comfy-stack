...
**Who this is for:** studio users who loaded `films/glasshouse/act-01` from Apps or Workflows.

> Generated from `workflows/_lab/films/glasshouse/act-01.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
This graph is **act 1/5** of a 7.5 min film (90 x 5.00s). Act 1 starts from Klein identity. In-graph stitch writes `ez_glasshouse_act1_90s.mp4`. After all five acts: `concat-shots.sh --film glasshouse --yes` -> `ez_glasshouse_450s.mp4`.
...
2. Load **films/glasshouse/act-01** from **Apps** or **Workflows**
...
**This graph:** `## films/glasshouse/act-01 LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invalid). The stitched MP4 is written automatically to `${COMFY_OUTPUT_DIR}` (container `/output...`
...
**This graph:** `films/glasshouse/act-01`
...
**This graph:** `## glasshouse (storm glasshouse - Rain on copper) Queue **once**. Klein identity still -> 18 x 5.00s LTX AV prints (last-frame continuity) -> **Save 90s film (MP4)**. Do not Queue a 90s denoise (121 ...`
...
**This graph (all 18 instances):** `films/glasshouse/act-01`
