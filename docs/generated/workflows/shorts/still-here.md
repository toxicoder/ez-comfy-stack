...
**Who this is for:** studio users who loaded `shorts/still-here` from Apps or Workflows.

> Generated from `workflows/_lab/shorts/still-here.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **shorts/still-here** from **Apps** or **Workflows**
...
Queue **once**. Klein identity still → 18 × 5.00s LTX AV prints (last-frame continuity) → **Save 90s film (MP4)**. Do not Queue a 90s denoise (121 frames = 1+8n per shot). Optional silent rehearsal: **wan/still-to-shot**. Optional host stitch: `./scripts/utilities/concat-shots.sh --film still-here --yes`.
...
**This graph (all 18 instances):** `shorts/still-here`
