...
**Who this is for:** studio users who loaded `ltx/still-to-video-5s` from Apps or Workflows.

> Generated from `workflows/_lab/ltx/still-to-video-5s.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
This graph is a smoke/demo (121 frames). For 90s films use ltx/still-to-shot (120 frames + last frame).
...
2. Load **ltx/still-to-video-5s** from **Apps** or **Workflows**
...
**This graph:** `## ltx/still-to-video-5s Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. LTX canvas 1280x704 (width/height must be divisible by 32; 720 and 1080 are invalid)…`
...
**This graph:** `ltx/still-to-video-5s`
