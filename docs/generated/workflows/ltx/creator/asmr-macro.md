...
**Who this is for:** studio users who loaded `ltx/creator/asmr-macro` from Apps or Workflows.

> Generated from `workflows/_lab/ltx/creator/asmr-macro.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
Quiet macro foley AV ~5 s. Lab size **1280×704**. Prefix `ez_asmr`.
...
2. Load **ltx/creator/asmr-macro** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Macro of an unmarked surface. Soft taps, close mic. No score. Five seconds. Kee…` |
...
**This graph:** `## ltx/creator/asmr-macro Quiet macro foley AV ~5 s. Lab size **1280×704**. Prefix `ez_asmr`. Empty of lettering. Add titles in your editor, not in the prompt. Occupancy: ltx — stop Wan, podcast, mus…`
...
**This graph:** `Macro of an unmarked surface. Soft taps, close mic. No score. Five seconds. Keep every object and surface from the start image; do not redesign.`

```text
Macro of an unmarked surface. Soft taps, close mic. No score. Five seconds. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `ltx/creator/asmr-macro`
