...
**Who this is for:** studio users who loaded `ltx/creator/tt-broll` from Apps or Workflows.

> Generated from `workflows/_lab/ltx/creator/tt-broll.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **ltx/creator/tt-broll** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Vertical B-roll from the start still. World SFX only. No score. Five seconds. K…` |
...
**This graph:** `## ltx/creator/tt-broll TikTok B-roll AV 9:16. Lab size **768×1280**. Prefix `ez_tt_broll`. Empty of lettering. Add titles in your editor, not in the prompt. Occupancy: ltx — stop Wan, podcast, music…`
...
**This graph:** `Vertical B-roll from the start still. World SFX only. No score. Five seconds. Keep every object and surface from the start image; do not redesign.`

```text
Vertical B-roll from the start still. World SFX only. No score. Five seconds. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `ltx/creator/tt-broll`
