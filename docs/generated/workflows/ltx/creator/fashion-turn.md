...
**Who this is for:** studio users who loaded `ltx/creator/fashion-turn` from Apps or Workflows.

> Generated from `workflows/_lab/ltx/creator/fashion-turn.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **ltx/creator/fashion-turn** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Slow turn from the start still. Cloth rustle. No score. Five seconds. Keep ever…` |
...
**This graph:** `## ltx/creator/fashion-turn Format / platform sets pixels (Custom uses Width × Height). Quality does not change size. Fashion turn AV 9:16. Lab size **768×1280**. Prefix `ez_turn`. Empty of lettering…`
...
**This graph:** `Slow turn from the start still. Cloth rustle. No score. Five seconds. Keep every object and surface from the start image; do not redesign.`

```text
Slow turn from the start still. Cloth rustle. No score. Five seconds. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `ltx/creator/fashion-turn`
