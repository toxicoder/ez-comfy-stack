...
**Who this is for:** studio users who loaded `ltx/creator/recipe-pour` from Apps or Workflows.

> Generated from `workflows/_lab/ltx/creator/recipe-pour.json`. Do not hand-edit this file. Re-run `python3 docs/generate_workflow_docs.py` (or `make docs`).
...
2. Load **ltx/creator/recipe-pour** from **Apps** or **Workflows**
...
| Instance | Value |
| --- | --- |
| Motion / prompt | `Slow pour into an unmarked bowl. Liquid, ceramic clink. No score. Five seconds.…` |
...
**This graph:** `## ltx/creator/recipe-pour Recipe pour AV 9:16. Lab size **768×1280**. Prefix `ez_pour`. Empty of lettering. Add titles in your editor, not in the prompt. Occupancy: ltx — stop Wan, podcast, music, o…`
...
**This graph:** `Slow pour into an unmarked bowl. Liquid, ceramic clink. No score. Five seconds. Keep every object and surface from the start image; do not redesign.`

```text
Slow pour into an unmarked bowl. Liquid, ceramic clink. No score. Five seconds. Keep every object and surface from the start image; do not redesign.
```
...
**This graph:** `ltx/creator/recipe-pour`
