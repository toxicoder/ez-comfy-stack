...
| Drive-through EDM arrangement scores | empty-body `[drop]`/`[inst]`/`[outro]` with cues inside the brackets, BPM; ninety-nine takes stay instrumental; two bass-set treats add one 1–2 word `[chorus]` chop (no `[verse]`). Free-text under a marker is sung |
