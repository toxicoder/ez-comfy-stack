...
- Negative CLIP nodes go through **Negative Prompt Enhance**, separate from **Rewrite prompt**. It stays **on** even when Rewrite prompt is off. It reads the final positive (the enhance output, the Prompt Join line, or every film shot through a prompt bundle) so canned `illustration` / `Pixar` terms cannot fight the look you actually queued. Families: klein, wan, ltx, zimage, longcat, dreamx, s2v. Distilled CFG 1 models still ignore the box; fold real exclusions into the **positive**, including a natural hand when a person is in frame.
...
- Sample prompt dropdown: 30 lab recipes for **this graph** plus **Custom** to type your own. Default is Custom so the canned widget text still Queues. Pick a sample to fill (and lock) the Prompt box; pick Custom to edit. **stills/dream-house** lists place recipes (Cliff villa, Forest cabin, …), not Klein still or Wan motion recipes. Queue still accepts a stamped label from another catalog so specialized graphs do not fail validation.
- After Queue, the dim **CLIP prompt** box is always visible and shows the string CLIP/ACE encoded
- Graph **Positive** (CLIPTextEncode; Wan I2V **Motion / prompt**) is a linked preview of that string: it follows Prompt / Rewrite prompt, then the rewritten CLIP text after Queue. Queue still encodes the Enhance STRING, not the preview widget.
...
- Optional style dropdown (300 presets): the rewriter weaves research-backed medium, light, color, and texture into the CLIP prompt, and retunes any style already in the source
...
<Tabs groupId="klein-4b-still-wan-2-2-t2v-wan-2-2-i2v-looping-gif-wan-i2v-dream-house-pack-klein-ltx-2-5-av-z-image-turbo-opt-in-wan-s2v-opt-in-longcat-video-opt-in-dreamx-creator-opt-in-ltx-ic-lora" items={["Klein 4B (still)", "Wan 2.2 T2V", "Wan 2.2 I2V", "Looping GIF (Wan I2V)", "Dream-house pack (Klein)", "LTX-2.5 AV", "Z-Image Turbo (opt-in)", "Wan S2V (opt-in)", "LongCat-Video (opt-in)", "DreamX-Creator (opt-in)", "LTX IC-LoRA"]} persist>
  <Tab value="Klein 4B (still)">
  ...
  <Tab value="Dream-house pack (Klein)">
  ...
  <Tab value="Z-Image Turbo (opt-in)">
  ...
  <Tab value="LongCat-Video (opt-in)">
...
| Node | Modes | Use on |
| --- | --- | --- |
| **Klein Prompt Enhance** | `t2i`, `edit`, `identity`, `text_swap`, `background_swap`, `background_edit` | every Klein still / edit / identity bible (including 90s film identity). `text_swap` is glyph-lock lettering on **stills/text-swap**. `background_swap` / `background_edit` are source-sized environment jobs on **stills/background-swap** and **stills/background-edit**. NVFP4 / base share this rewriter |
...
| **Negative Prompt Enhance** | family `klein` `wan` `ltx` `zimage` `longcat` `dreamx` `s2v` | every CLIP **Negative**. App label **Rewrite negative**. Reads the final positive so the negative cannot fight it. Stays on when Rewrite prompt is off |
...
2. Pick a **Sample prompt** (30 lab recipes) or **Custom** to type a lazy sentence (the canned paragraph stays until you pick a sample). **Enhance** defaults **on**.
3. Optional: pick a **style** (photorealistic, anime, cartoon, … — 300 ids, or `none`).
4. Queue. On the Enhance node, read the dim **CLIP prompt** box — that is the text CLIP encoded. The Graph **Positive** node (CLIPTextEncode) is the same preview. The top prompt widget stays as you typed it. If the 4B rewriter was skipped, **Enhance status** says why (missing GGUF, missing llama.cpp, timeout) and generation still runs. A selected style should read as that medium (cel, watercolor, oil on canvas, …), not a 3D/photo paragraph with a style trailer. Rewrite prompt **off** copies the Prompt box onto Positive immediately; **on** waits for Queue for the 4B rewrite (Positive still shows the source until then).
...
| **motion/av/dialogue-8s** / **multishot-5s** / **product-hero** / **first-last-5s** / **audio-to-video-5s** | Authored LTX-2.5 showcase; quoted speech, named cuts, freeze bed |
...
| Drive-through EDM arrangement scores | empty-body `[drop]`/`[inst]`/`[outro]` with cues inside the brackets, BPM; all one hundred one takes stay instrumental, and each score is fitted to ACE-Step's 2048-token lyric window. Free-text under a marker is sung |
...
- **Rewrite negative** takes that final CLIP string as context, including when Rewrite prompt is off (the string is the text you typed). A canned `illustration` / `Pixar` negative is dropped when the positive is watercolor or stylized feature 3D. Anatomy terms such as extra fingers stay unless the positive asked for that body. Watermarks and melt/flicker stay. Distilled Klein, Z-Image Turbo, Wan 5B MagCache drafts, and distilled LTX are CFG 1 (the box is ignored); the positive has to name a natural hand. Klein base, Wan A14B, S2V, LongCat, and DreamX read the list. Turning Rewrite negative off skips the LLM only; the conflict filter still runs.
...
<Callout type="warning" title="GPU-first local LLM">
...
Queue **stills/still-draft** first ([Getting Started](getting-started.md)). Canvas Apps (still-draft, still-hero, Wan/LTX 5 s, creator jobs) expose **Format / platform**. **stills/still-studio** is the still desk where prefix follows Format and Look recipe is on. **stills/image-studio** is the universal still desk: 100 creator modes, Format / platform, and an optional example/reference still (Queue without a file is valid). **This run** (under **Iterate**) is the mode, prompt, size, prefix, and sampler values Queue will send — a named Sample wins over the prompt box, and Rewrite on means that box is the rewriter source, not the CLIP text. **Iterate** off keeps creator mode. **Iterate** on: the first Run is text-to-image; the saved still is fed back and the next Run is an edit you can retune. Clear the reference to start from text again. **stills/character-draft** then **stills/character-tweak** iterates a still. Swap lettering on an existing plate with **stills/text-swap** (output matches the source size; style is off). Replace the environment (including ground) with **stills/background-swap** (named samples skip the rewriter; Cubic block world rebuilds this photographed place from a block study and pastes the original people), or restyle it in place with **stills/background-edit**. Daily loop: still → Wan 5 s → LTX 5 s on [Visual Generative AI](visual-generative-ai.md).
