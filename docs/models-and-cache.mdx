...
- **Check models** on every lab App (Queue skips it; click for the host download command)
...
- **Checking the tree** without network access - including **Check models** on every lab App (Queue does not run that node)
...
| Operator JSON saved under `_lab/` | same tree | same | **rescue** extras into `_user/` and explicit in-place Saves into `_user/_rescued/`, then lab overwrite. Catalog restamps do **not** clone `_lab` |
...
Every lab App also has an isolated **Check models** node (and an occupancy-chip button in App Mode). Queue does not run it. Click it to see whether this graph plus the current Quality combo is on disk; missing files print the host command above. Restart Comfy after a pull so `models/*` re-link.
