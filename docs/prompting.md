...
- Sample prompt dropdown: 20 lab recipes for **this graph** plus **Custom** to type your own. Default is Custom so the canned widget text still Queues. Pick a sample to fill (and lock) the Prompt box; pick Custom to edit. **stills/dream-house** lists place recipes (Cliff villa, Forest cabin, …), not Klein still or Wan motion recipes. Queue still accepts a stamped label from another catalog so specialized graphs do not fail validation.
...
| Node | Modes | Use on |
| --- | --- | --- |
| **Klein Prompt Enhance** | `t2i`, `edit`, `identity`, `text_swap` | every Klein still / edit / identity bible (including 90s film identity). `text_swap` is glyph-lock lettering on **stills/text-swap**. NVFP4 / base share this rewriter |
...
| **motion/av/dialogue-5s** / **multishot-5s** / **product-hero** / **first-last-5s** / **audio-to-video-5s** | Authored LTX-2.5 showcase; quoted speech, named cuts, freeze bed |
...
Queue **stills/still-draft** first ([Getting Started](getting-started.md)). Canvas Apps (still-draft, still-hero, Wan/LTX 5 s, creator jobs) expose **Format / platform**. **stills/still-studio** is the still desk where prefix follows Format and Look recipe is on. **stills/image-studio** is the universal still desk: 100 creator modes, Format / platform, and an optional example/reference still (Queue without a file is valid). **stills/character-draft** then **stills/character-tweak** iterates a still. Swap lettering on an existing plate with **stills/text-swap** (output matches the source size; style is off). Daily loop: still → Wan 5 s → LTX 5 s on [Visual Generative AI](visual-generative-ai.md).
