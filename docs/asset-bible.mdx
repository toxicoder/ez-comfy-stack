...
`mcp-construct` is the in-tree [Blender MCP](occupancy.md) (`manage.sh blender-mcp`). Typed tools only - no `execute_code`. **`bpy-primitive`** now has one operator verb: `./scripts/manage.sh house-views` builds a greybox set under `assets/sets/<slug>/` (layout + GLB + ten 1024x1280 clay stills). Host Blender stays a [sidecar](blender-gb10-sidecar.md) (dies if compose is up). Playbook: [Dream-house tours](learn/dream-house.md).
