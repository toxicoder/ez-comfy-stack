...
- Occupancy (one GB10 job) — chip at the top of the widget list, plus App summary and Quality caption
...
Lab graphs are the host `_lab/<lane>/*.json` files (folder-scoped ids). On `start` they seed into Comfy as `*.app.json` when App Mode is the default view, so they appear under **Apps** as well as **Workflows**. [App Mode](learn/comfyui.md#graph-and-app) is a widget surface on that JSON (ComfyUI frontend **1.41.13+**). It is **not** a second frontend and not [`studio-ui`](reference/studio-ui.md). A small occupancy chip sits at the top of the App widget list (under the menu if you are in graph view) and shows which family is running, a one-line summary of what the App does (`extra.lab_description`), the active Quality overlay, still N of M on multi-plate Apps, and the next handoff. Widget help text wraps so the full description stays readable. It does not cover Run. Restart the container after a pull so `custom_nodes/ez_studio_app` is copied.
...
| `ltx` | any AV 12 s App | Wan, podcast, music, other LTX |
...
| **motion/av/still-to-video-12s** | ltx | AV 12 s, 1280×704 |
| **motion/av/hook-av** | ltx | AV cold open |
| **motion/av/dialogue-12s** | ltx | Quoted speech + world SFX. Prefix `ez_ltx_dialogue` |
| **motion/av/multishot-12s** | ltx | Native multishot (named cuts). Prefix `ez_ltx_multishot` |
| **motion/av/product-hero** | ltx | Packshot I2V orbit. Prefix `ez_ltx_product` |
| **motion/av/first-last-12s** | ltx | First + last still → one AV take. Prefix `ez_ltx_flf` |
| **motion/av/audio-to-video-12s** | ltx | Freeze a ~12 s wav; mux original audio. Prefix `ez_ltx_a2v` |
...
| **dcc/depth-control-12s** | ltx | Union Control envelope, depth from `depth.mp4` |
| **dcc/canny-control-12s** | ltx | Same envelope, canny from `canny.mp4` |
...
| From | To |
| --- | --- |
| Spark Still | Hero Still → Silent 5s (`wan-i2v-5s`) → AV 12s (`ltx-i2v-5s`) or FLF (`motion/av/first-last-12s`) |
| Product packshot | `motion/av/product-hero` |
| Talking-head still | `motion/av/audio-to-video-12s` (real freeze; two-stage stays Templates) |
...
| Shorts thumb (`creator/stills/youtube-shorts-thumb`) | `creator/silent/zoom-punch` → `motion/av/shorts-still-12s` |
