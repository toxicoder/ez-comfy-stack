...
| App | What it does |
| --- | --- |
| **stills/still-draft** | Spark Still. Default 768x432; **Format / platform** retargets (YouTube, Shorts, ...). Prefix `ez_still_draft` |
| **stills/identity-sheet** | Front / three-quarter / profile. 1280x704, Enhance on (identity mode) |
...
| **stills/character-draft** | Character still. Prompt + style, 1024x1280, prefix `ez_character` |
...
| **stills/still-studio** | klein | Format / platform desk. Prefix follows Format (`ez_still_studio` when Custom). Default 1280x704 |
| **stills/image-studio** | klein | Universal still desk. **Iterate** (first control, off by default) does text-to-image, then edits that still. **This run** shows the values Queue will send. 100 creator modes + Format / platform + optional reference. Prefix follows Creator mode unless Iterate is on (`ez_iterate`). Default 1280x704 |
| **stills/still-hero** | klein | `ez_still_hero` - default 1280x704 LTX feeder; Format / platform retargets |
| **stills/thumbnail** | klein | `ez_thumbnail` 1280x720 |
| **stills/instagram-square** | klein | `ez_ig_square` 1:1 |
| **stills/open-graph** | klein | `ez_og` 1216x640 |
| **stills/banner-wide** | klein | `ez_banner` ~3:1 |
| **stills/shorts-still** | klein | `ez_shorts_still` 432x768 |
...
| **motion/av/still-to-video-8s** | ltx | AV 8 s, 1280x704 |
...
| **stills/background-swap** | klein | `ez_bg_swap` - replace backdrop, ground, and nearby set dressing; empty Flux.2 canvas; photo reference for place swaps; Cubic block world uses a people-erased block study and pastes the people back at full resolution. Other/background character toggles |
...
| --- | --- | --- |
| **dcc/clay-hero** | klein | Klein edit of guide `first.png`, 1280x704, prefix `ez_clay_hero` |
| **dcc/clay-plates** | klein | One clay still -> hero 704 / packshot 1:1 / IG 4:5 / shorts 9:16 (`ez_clay_pack_*`) |
| **dcc/canny-hero** | klein | Klein edit of `canny.png`, 1280x704, prefix `ez_canny_hero` |
...
| **dcc/depth-control-shorts** | ltx | Depth envelope at **768x1280** |
