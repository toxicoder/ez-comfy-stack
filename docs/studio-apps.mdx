...
| **motion/av/still-to-video-10s** | ltx | AV 12 s, 1280×704 |
| **motion/av/hook-av** | ltx | AV cold open |
| **motion/av/dialogue-10s** | ltx | Quoted speech + world SFX. Prefix `ez_ltx_dialogue` |
| **motion/av/multishot-10s** | ltx | Native multishot (named cuts). Prefix `ez_ltx_multishot` |
| **motion/av/product-hero** | ltx | Packshot I2V orbit. Prefix `ez_ltx_product` |
| **motion/av/first-last-10s** | ltx | First + last still → one AV take. Prefix `ez_ltx_flf` |
| **motion/av/audio-to-video-10s** | ltx | Freeze a ~12 s wav; mux original audio. Prefix `ez_ltx_a2v` |
...
| **dcc/depth-control-10s** | ltx | Union Control envelope, depth from `depth.mp4` |
| **dcc/canny-control-10s** | ltx | Same envelope, canny from `canny.mp4` |
...
| From | To |
| --- | --- |
| Spark Still | Hero Still → Silent 5s (`wan-i2v-5s`) → AV 12s (`ltx-i2v-5s`) or FLF (`motion/av/first-last-10s`) |
| Product packshot | `motion/av/product-hero` |
| Talking-head still | `motion/av/audio-to-video-10s` (real freeze; two-stage stays Templates) |
...
| Shorts thumb (`creator/stills/youtube-shorts-thumb`) | `creator/silent/zoom-punch` → `motion/av/shorts-still-10s` |
