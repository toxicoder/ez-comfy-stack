...
| `wan` / `ltx` | up, 90g/80g | video | **stopped** | Wan 5s / LTX App 8s |

App occupancy keys (`none` / `llm` / `klein` / ...) on Prompt Forge and research-chat are **graph labels**. `llm` means nothing GPU. It is not `occupancy enter llm`. Mode x legal GPU jobs: [Occupancy matrix](operate/occupancy-matrix.md).
