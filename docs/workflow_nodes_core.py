...
def core_nodes() -> dict[str, Any]:
    ...
    return {
        ...
        "EZVideoFormat": _n(
            ...
            origin="ez_image",
            lab="Wan and LTX printers wire width/height into Wan22ImageToVideoLatent, LTXVImgToVideo, or EmptyLTXVLatentVideo. Hint feeds Enhance duration_hint on non-loop graphs. Length stays on the latent node. Quality does not change size. Match input snaps aspect to a loaded still.",
            sockets=[
                _s("image", "IMAGE", "in", "Optional still used when Output size is Match input."),
