...
def core_nodes() -> dict[str, Any]:
    ...
    return {
        ...
        "EmptyFlux2LatentImage": _n(
            ...
            lab="Draft 768x432 batch 2. Hero / LTX feeders 1280x704. Portrait 1024x1280 or 768x1280. 1280x720 is OK for thumbnails, not for LTX feeders.",
            ...
            widgets=[
                ...
                _w("height", index=1, typ="INT", desc="Latent pixel height.", gen="1280x704 is the LTX VAE grid (div32). 1280x720 is not."),
        ...
        "EmptyLTXVLatentVideo": _n(
            ...
            lab="Width/height must be div32. Length must be 1+8n (193 for ~8 s Apps; 121 for 5 s film printers). ez_ltx_spatial snaps illegal values.",
            sockets=[_s("LATENT", "LATENT", "out", "Video latent (then concat with audio latent).")],
            widgets=[
                _w("width", index=0, typ="INT", rng="1280 (landscape) / 768 (shorts)", desc="Frame width.", gen="div32. 1280x720 will snap to 704."),
        ...
        "Wan22ImageToVideoLatent": _n(
            ...
            lab="Smoke 832x480 x 121 frames. Shot graphs use 120 frames for concat. GIF 49 frames. VACE join 17 frames (1+8n).",
            sockets=[
            ...
            widgets=[
                _w("width", index=0, typ="INT", rng="832 landscape / 480 portrait", desc="Frame width.", gen="832x480 is the Wan 5B smoke size. Larger melts GB10."),
                _w("height", index=1, typ="INT", desc="Frame height.", gen="Swap for 9:16 shorts (480x832)."),
        ...
        "EZEmptyFlux2FromImage": _n(
            ...
            origin="ez_image",
            lab="stills/background-swap uses this as KSampler.latent_image. The encoded source is a reference (photo, or a block study for Cubic block world), not the denoise start.",
            sockets=[
                _s("image", "IMAGE", "in", "Snapped still."),
                _s("LATENT", "LATENT", "out", "Empty Flux.2 noise canvas (div16, 128 channels)."),
            ],
            widgets=[],
        ),
        "EZImageUpscale": _n(
            ...
            widgets=[
                _w("upscale", index=0, typ="COMBO", rng="none / 2x / 4x / 4K", desc="Upscale mode.", gen="none is a passthrough. 2x and 4x are lanczos. 4K fits the still in a 3840x2160 box (portrait 2160x3840). No extra weights.", choices=[("none", "Pass through."), ("2x", "Double pixels."), ("4x", "Quadruple pixels."), ("4K", "Fit in a 4K box.")]),
            ],
        ),
        "EZMatchImageSize": _n(
            "Match image size",
            ...
            sockets=[
                ...
                _s("size_src", "IMAGE", "in", "Source still whose HxW is the target."),
        ...
        "EZCubicCondition": _n(
            ...
            sockets=[
                ...
                _s("image", "IMAGE", "in", "Snapped source still."),
                _s("vae", "VAE", "in", "Flux.2 VAE."),
                _s("prompt", "STRING", "in", "Enhance STRING. A link, not an App widget."),
                _s("CONDITIONING", "CONDITIONING", "out", "Conditioning with one reference latent."),
            ],
            widgets=[],
        ),
        "EZReinsertPeople": _n(
        ...
        "EZImageFormat": _n(
            ...
            sockets=[
                ...
                _s("width", "INT", "out", "Latent width (div16)."),
                _s("height", "INT", "out", "Latent height (div16)."),
            ...
            widgets=[
                _w("format", index=0, typ="COMBO", rng="16:9 LTX feeder / platform jobs / Custom", desc="Aspect or named platform job.", gen="Preset writes pixels, save prefix, and Rewrite prompt framing. Custom uses Width x Height (snapped to div16, max 2048). Does not change Quality, CLIP, or VAE.", choices_from="image_formats"),
        ...
        "EZImageMode": _n(
            ...
            origin="ez_image",
            lab="stills/image-studio wires context into EZKleinPromptEnhance, enhance_mode into the Enhance mode widget, and prefix into SaveImage. Optional references stay optional. Iterate forces text-to-image, then edit once has_image is set.",
            sockets=[
                _s("context", "STRING", "in", "Optional look-recipe splice from EZImageFormat."),
                _s("has_image", "BOOLEAN", "in", "Presence flag from EZOptionalImage. Force input. Off while Iterate is on means the next Run is text-to-image."),
                _s("context", "STRING", "out", "Mode instruction plus incoming look splice, or the Iterate edit line."),
                ...
                _s("prefix", "STRING", "out", "SaveImage filename prefix. ez_iterate while Iterate is on."),
            ],
            widgets=[
                _w("category", index=0, typ="COMBO", rng="Generate / Scene / Subject / ...", desc="Filter Creator mode.", gen="JS hides modes outside this category. Python run() uses Mode even if Category is stale. Ignored for mode and prefix while Iterate is on.", choices_from="image_mode_categories"),
                _w("mode", index=1, typ="COMBO", rng="Photoreal still / Background swap / Change text / ...", desc="Creator preset.", gen="Sets Enhance mode, save prefix, and a locked instruction. Empty reference stills never error. Ignored while Iterate is on.", choices_from="image_modes"),
                _w("iterate", index=2, typ="BOOLEAN", rng="false", desc="Text-to-image, then edit the saved still.", gen="Off keeps creator mode. On: no reference is t2i (prefix ez_iterate); a reference is an edit. The App copies the saved still into the reference after Run. Clear the reference to start from text again."),
                _w("run_summary", index=3, typ="STRING", rng="multiline", desc="This run. Values Queue will send.", gen="Display only. Python run() ignores it. The App fills it from POST /ez_image/studio-preview before Run."),
            ],
        ),
        "EZOptionalImage": _n(
        ...
        "EZVideoFormat": _n(
            ...
            sockets=[
                _s("image", "IMAGE", "in", "Optional still used when Output size is Match input."),
                _s("width", "INT", "out", "Latent width (Wan div16, LTX div32)."),
                _s("height", "INT", "out", "Latent height (Wan div16, LTX div32)."),
                _s("hint", "STRING", "out", "Enhance duration / framing line."),
                _s("prefix", "STRING", "out", "Optional filename prefix (often unwired)."),
            ],
            widgets=[
                _w("family", index=0, typ="COMBO", rng="Wan 5B / LTX-2.5", desc="Which VAE grid to use.", gen="Wan snaps div16 (max 1024). LTX snaps div32 (max 1280). App Mode hides this - occupancy already picks the model.", choices_from="video_families"),
                _w("format", index=1, typ="COMBO", rng="16:9 YouTube / 9:16 Shorts / Custom", desc="Aspect or named platform job.", gen="Preset writes pixels and Rewrite prompt framing. Custom uses Width x Height. Does not change Quality, length, CLIP, or VAE.", choices_from="video_formats"),
