...
<Callout type="warning" title="HF_TOKEN is not the LTX license click">
...
<Callout type="warning" title="Unthrottled wrap can freeze SSH">
...
| `v ... 0 MiB/s` + `found N incomplete` + lock still held | Hung **resume** - live `hf` holds the lock, partial not growing | ++ctrl+c++. Do **not** FORCE-clear locks while it runs. Then `./scripts/manage.sh reset-hf-partials --yes` and re-run, or `./scripts/manage.sh download-models --drop-incomplete`. Finished `.safetensors` are kept |
...
| No `v` / `══ n/m ══` progress line | Piped SSH, `EZ_COMFY_PROGRESS=0`, or `HF_PROGRESS=0` | Progress rewrites a TTY line; pipes get a newline every interval. Unset those env vars. `LAB_DEBUG=1` for extra traces |
