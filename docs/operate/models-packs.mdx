...
**Who this is for:** operators after `MODELS_DIR` is writable. Layout: [Models and cache](../models-and-cache.md). Gated LTX: [HF_TOKEN](models-tokens.md). Sharing: [Multi-stack sharing](models-sharing.md). Pack, size, and the download command: [Download tiers](../download-tiers.md).

## Download

`download-models` has **no** `--tier`. It always pulls the default still + Wan 5B + LTX-2.5 set. On every other downloader, `--tier` is **which pack**, not a global quality ladder. `--limit` is Mbps. The one-screen list of packs, sizes, and commands is [Download tiers](../download-tiers.md).
...
`--tier fast` also pulls Klein companions (`te` + `vae`). Opt-in packs (Wan A14B, Fun InP ~47 GB, LTX 2.3, music, podcast, 3D, ...) are rows on [Download tiers](../download-tiers.md).
...
Opt-in packs are not part of `download-models`. Commands and rough sizes are the table on [Download tiers](../download-tiers.md). Fun InP is ~47 GB.
...
| Graph | Notes |
| --- | --- |
| `stills/still-draft.json` | Klein 4B 768x432, 4 steps, batch 2 |
| `audio/dub/clone-translate.json` | Multi-speaker clone-and-translate (`ez_dub_mix` / `ez_dub_yt`; opt-in dub pack) |
| `audio/music/rap-draft.json` | ACE-Step rap cold-open draft (`ez_rap_draft`; opt-in AIO) |
| `audio/music/rap-full.json` | ACE-Step rap pre-chorus full take (`ez_rap_full`) |
| `_lab/audio/albums/nill-bye/<album>/` | Nine Nill Bye albums (135 ACE-Step takes, 64-210 s, varied form + `cover.json` + `album.json`). SaveAudio stem `NN - Song Title`. Catalog: [Local music](../music.md) |
| `_lab/audio/albums/drive-through/<album>/` | Six Drive-through albums (101 ACE-Step bass-set EDM takes, 150-480 s, short build then switch-ups, tempos 165-176 + cover + album pack; all instrumental). SaveAudio stem `NN - Song Title`. Catalog: [Local music](../music.md) |
| `stills/still-hero.json` | Same prompt/seed, 1280x704 (LTX VAE grid) |
