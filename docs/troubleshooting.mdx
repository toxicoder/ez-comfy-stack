...
- **Topic pages** - docs site, canvas, host/Docker, downloads, start/runtime, models/workflows, guide packs
...
<Cards>
  <Card title="Docs site">
    ---

    Copy-paste still showing `${SPARK_HOST}` / `${MODELS_DIR}` chips.

    [Docs site](operate/troubleshooting-docs-site.md)
  </Card>
  <Card title="Studio canvas">
    ---
    ...
    [Studio canvas](operate/troubleshooting-canvas.md)
  </Card>
  <Card title="Host and Docker">
    ---

    Docker missing, `MODELS_DIR` / `COMFY_OUTPUT_DIR` permission, Spark farm SSH (`SPARK_HOSTS` / `SPARK_FABRIC_IPS`).

    [Host and Docker](operate/troubleshooting-host-docker.md)
  </Card>
  <Card title="Downloads">
    ...
    [Downloads](operate/troubleshooting-downloads.md)
  </Card>
  <Card title="Start and runtime">
    ---

    Headroom refuse, Kitchen fallback, cold start, GHCR / layer cache.

    [Start and runtime](operate/troubleshooting-start-runtime.md)
  </Card>
  <Card title="Models and workflows">
    ---

    Empty models, CLIP/VAE links, 90s concat, dream-house, ACE, occupancy gate.

    [Models and workflows](operate/troubleshooting-models-workflows.md)
  </Card>
  <Card title="Guide packs">
    ---

    Animatic or the film board shows missing clay after `blender-guide --film go-see`. Packs live at `guides/go-see/...`.

    [DCC workflows](dcc-workflows.md)
  </Card>
</Cards>
...
| Guide packs | [DCC workflows](dcc-workflows.md) |
...
| Dub clone does not sound like the speaker | The reference window was clipped, tonal, or off-speaker, or every window was rejected | Open `dubs/<slug>/speakers/spkNN.ref.json` (also under `qc.json` `reference`). `score` is the window rank. `degraded: true` / Dub status `ref: degraded` means the loudness fallback ran; `reject` names the gate (`clip`, `tonal`, `band`, `unvoiced`, `not_speech`). Overlap and quiet turns are still dropped. Re-Queue Stage **render** after the source is one person without a music bed. |
...
flowchart TB
  ...
  Q --> Slow{"Extreme thrash<br/>5-15x slow?"}
