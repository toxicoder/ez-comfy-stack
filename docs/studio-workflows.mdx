...
Reusable printer blocks live in the node library after start (`custom_nodes/ez_studio_blocks/subgraphs/`): **klein-t2i-backbone**, **wan-i2v-5s**, **ltx-av-10s**, **ltx-film-shot**. Drop them from the subgraph menu instead of copy-pasting chains.
...
flowchart TB
  ...
  AV --> L["motion/av/still-to-video-10s or shorts"]
