...
| `EZEmptyFlux2FromImage` | Empty Flux.2 from image | `image` IMAGE | `LATENT` latent | — | Empty Flux.2 noise canvas at the still's snapped W×H. **stills/background-swap** denoise start; encoded source stays on ReferenceLatent |
