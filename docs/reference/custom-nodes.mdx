...
| `EZImageMode` | Creator mode | `category` combo, `mode` combo (100 labels), optional `context` STRING | `STRING` context/enhance_mode/prefix | klein | stills/image-studio. Category filters Mode. Queue splices an instruction, picks t2i/edit/text_swap/identity/background_swap/background_edit, and sets the save prefix. Optional refs never required |
