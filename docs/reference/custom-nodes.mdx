...
| `EZCubicCondition` | Cubic or photo reference | `conditioning`, `latent` (photo encode), `image`, `vae`, `prompt` STRING forceInput | `CONDITIONING` | — | **stills/background-swap**. Place swaps attach the photo. Cube / voxel / block-world prompts attach a coarse block study of that photo |
| `EZReinsertPeople` | Reinsert people | `plate` IMAGE, `source` IMAGE, `prompt` STRING forceInput | `IMAGE` image | — | Pastes detected people from the source onto a cubic rebuild. Other prompts pass the plate through. Missing BSD-3 DeepLab weights fail soft (`EZ_PERSON_MASK=off` skips the paste) |
