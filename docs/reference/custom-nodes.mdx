...
| Class | Display name | Inputs | Outputs | Occupancy | QC / rights |
| --- | --- | --- | --- | --- | --- |
| `EZImageDescribe` | Describe image | `enable` BOOLEAN (default off); optional lazy `image` IMAGE | `STRING` caption | CPU VLM (not a CLI occupancy mode) | Off (default) returns empty and does not load the describe GGUF. On captions a source still into Enhance `image_desc`. Opt-in weights: `download-llm --tier describe` (Qwen2.5-VL-3B Apache). Not `EZDescribeImage` |
