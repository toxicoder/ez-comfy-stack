...
<Callout type="error" title="Do not commit secrets">
...
<Callout type="warning" title="Do not weaken headroom">
...
| `schemas/guide_pack.shot.yaml` | `ez.guide.shot.v1` DCC \<-> Comfy print handshake (120 frames @ 24 fps) | `export-guides` / `scripts/lib/guide_pack.py` | Size is LTX VAE grid **1280x704** or **768x1280**. Never 1280x720 |
| `schemas/guide_pack.still.yaml` | `ez.guide.still.v1` single-frame clay/depth/canny plates | `blender-stills` | Allowed sizes: 1280x704, 768x1280, 1024x1280, 1024x1024, 1280x720 (**Klein thumb only**, never LTX) |
