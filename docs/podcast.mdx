...
| COVER | Queue **stills/podcast-cover** separately (1024^2, `ez_podcast`) | `ez_podcast` |
...
Native ACE-Step 1.5 already ships in `COMFYUI_REF=v0.37.0` (`TextEncodeAceStepAudio1.5`, `EmptyAceStep1.5LatentAudio`). Beds stay **instrumental** (`instrumental`, `no vocals` in tags; empty lyrics). Rap vocals with original lyrics live on [Local music](music.md) and reuse this same AIO dest - do not download it twice.
