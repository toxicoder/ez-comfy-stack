...
3. **Motion** — same PNG into **motion/av/still-to-video-8s** (LTX-2.5 I2V). Prompt only the motion and sound. Let LTX’s own 2× upscaler / DFR Templates handle resolution.
...
4. Optional audio: **motion/av/still-to-video-8s**, same first frame, same seed note, Queue ~5 s AV at **1280×704**.
