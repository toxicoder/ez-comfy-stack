...
- Keep this site’s `.md-typeset { font-size: 0.875rem }`, sticky table pin (`.ez-table-pin`), and floating horizontal scrollbar (`.ez-table-hscroll` in `tables.js`). Both hide once the table has scrolled past the viewport. Do not copy spark-lab’s `0.82rem` typeset or table `overflow: hidden`. Do not `position: sticky` the header or the h-scroll bar.
