...
- **`deploy-docs.yml`** — `main` → `latest`, `development` → `development` (two Next exports)
...
PR validation is `make docs` in CI only (no Pages push). Deploy sets `EZ_DOCS_PUBLISHED_AT` for the last-published chip. `docs-site/lib/site.ts` stamps `__DOCS_GIT_REF__` and GitHub blob/tree URLs to the long-lived ref.
