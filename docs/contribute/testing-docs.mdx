...
- **`bazelisk run //docs:docs`** — generators then Fumadocs `next build --webpack` into `docs-site/out/` (MDX compiles on demand; webpack does not bundle the ~850-page corpus)
...
| Target | What it runs |
| --- | --- |
| **`bazelisk test //:test-fast`** / **`make test`** | BATS + Python (`pytest-xdist` when installed) + Pyright + mypy |
...
- `docs` generators (`commands.py`, `glossary.py`, `page_brief.py`, workflow/cinema/audio/shell)
- `scripts/lib`
- `studio-ui`
